...
function KeyboardProduct($location, $query, $pricediscount, $datakeyboard, $statuscustom = false, $backuser = "backuser", $valuetow = null, $customvolume = "customsellvolume", $queryParams = [])
{
    ...
    // Header: Price (green) | Product (blue) — glass style
    $product['inline_keyboard'][] = [
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (is_array($hide_panel) && in_array($location, $hide_panel))
            continue;
        ...
        $cb = "{$datakeyboard}{$result['code_product']}{$valuetow}";

        // Left: Price (green) | Right: Product name (blue)
        $product['inline_keyboard'][] = [
            ['text' => $product_price, 'callback_data' => $cb, 'style' => 'success'],
            ['text' => $product_name, 'callback_data' => $cb, 'style' => 'primary']
        ];
    }

    // Custom volume row (same glass columns)
    if ($statuscustom) {
        $custom_volume_price = select("shopSetting", "*", "Namevalue", "customvolmef", "select")['value'] ?? '4000';
        if (empty($custom_volume_price)) {
            $custom_volume_price = "4000";
        }
        $product['inline_keyboard'][] = [
            ['text' => number_format((float)$custom_volume_price) . ' تومان', 'callback_data' => $customvolume, 'style' => 'success'],
    ...
    // Back button (red)
    $product['inline_keyboard'][] = [
        ['text' => $textbotlang['users']['status']['backinfo'] ?? '🔙 بازگشت', 'callback_data' => $backuser, 'style' => 'danger']
    ];
