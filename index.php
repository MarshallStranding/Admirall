...
if ($text == "/start" || $datain == "start" || $text == "start") {
    ...
    // 🔵 = active/connected, 🔴 = inactive/expired/limited, 🟡 = warning, ⚪ = on_hold/unlimited
    $status_icons = [
        'active'        => '🟢',
        'end_of_time'   => '🔴',
        'end_of_volume' => '🔴',
    ...
    if ($setting['statusnamecustom'] == 'onnamecustom') {
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $data = "";
            if ($row != null && !empty($row['note']))
                $data = " | {$row['note']}";
            $icon = $status_icons[$row['status']] ?? '⚪';
            $keyboardlists['inline_keyboard'][] = [
                [
                    'text' => $icon . " " . $row['username'] . $data,
    ...
    // Add sub link import option
    $keyboardlists['inline_keyboard'][] = [['text' => '🔗 افزودن سرویس از لینک ساب', 'callback_data' => 'addservice_sublink']];
    ...
    Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['service_sell'], $keyboard_json);
} elseif ($datain == "addservice_sublink") {
    // New: add service via subscription link
    $msg = "🔗 <b>افزودن سرویس از لینک ساب</b>\n\nلینک اشتراک (Subscription URL) سرویس خود را ارسال کنید.\n\n⚠️ لینک ساب معمولاً با <code>https://</code> شروع می‌شود.";
    $cancel_kb = json_encode(['inline_keyboard' => [[['text' => '❌ انصراف', 'callback_data' => 'backorder']]]]);
    sendmessage($from_id, $msg, $cancel_kb, 'html');
    step('get_sublink_import', $from_id);
} elseif ($user['step'] == "get_sublink_import") {
    if (empty($text)) return;
    $sublink_input = trim($text);
    // Validate URL format
    if (!preg_match('#^https?://#i', $sublink_input)) {
        sendmessage($from_id, "❌ لینک وارد شده معتبر نیست.\nلینک باید با <code>https://</code> شروع شود.", null, 'html');
        return;
    }
    // Normalize URL for comparison
    $sublink_normalized = rtrim($sublink_input, '/');
    $sub_host = parse_url($sublink_input, PHP_URL_HOST) ?? '';
    $sub_path = parse_url($sublink_input, PHP_URL_PATH) ?? '';

    // Extract username candidates from URL path
    $path_parts = array_values(array_filter(explode('/', $sub_path)));
    // The username is usually the last meaningful path segment (after /sub/ or similar)
    $candidate_usernames = [];
    foreach ($path_parts as $part) {
        if (strlen($part) >= 3 && !in_array(strtolower($part), ['sub', 'subscription', 'api', 'v1', 'v2', 'panel'])) {
            $candidate_usernames[] = $part;
        }
    }

    $found_user = null;
    $found_panel = null;
    $panels = $pdo->query("SELECT * FROM marzban_panel WHERE status='active'")->fetchAll(PDO::FETCH_ASSOC);

    // Method 1: Try to find by exact subscription_url match in panel
    foreach ($panels as $panel) {
        $panel_host = parse_url($panel['url_panel'], PHP_URL_HOST) ?? '';
        // Skip if host doesn't match panel host (quick pre-filter)
        // Note: sub link host may differ from panel host, so we try all
        foreach ($candidate_usernames as $try_username) {
            $check = $ManagePanel->DataUser($panel['name_panel'], $try_username);
            if (!isset($check['username']) || $check['status'] == 'Unsuccessful') continue;
            $panel_sub = rtrim($check['subscription_url'] ?? '', '/');
            if ($panel_sub === $sublink_normalized) {
                $found_user = $check;
                $found_panel = $panel;
                break 2;
            }
    ...
    // Method 2: Try host-based match (panel host == sub link host) with username in path
    if ($found_user === null) {
        foreach ($panels as $panel) {
            $panel_host = parse_url($panel['url_panel'], PHP_URL_HOST) ?? '';
            if ($panel_host !== $sub_host) continue;
            foreach ($candidate_usernames as $try_username) {
                $check = $ManagePanel->DataUser($panel['name_panel'], $try_username);
                if (!isset($check['username']) || $check['status'] == 'Unsuccessful') continue;
                $found_user = $check;
    ...
    // Method 3: Check invoice table by user_info field (previously imported sub links)
    if ($found_user === null) {
        $stmt_sub = $pdo->prepare("SELECT * FROM invoice WHERE user_info LIKE ? LIMIT 1");
        $stmt_sub->execute(['%' . $sub_host . '%']);
        $existing_invoice = $stmt_sub->fetch(PDO::FETCH_ASSOC);
        if ($existing_invoice) {
            foreach ($panels as $panel) {
                $check = $ManagePanel->DataUser($panel['name_panel'], $existing_invoice['username']);
                if (isset($check['username']) && $check['status'] != 'Unsuccessful') {
                    $panel_sub = rtrim($check['subscription_url'] ?? '', '/');
                    if ($panel_sub === $sublink_normalized || strpos($panel_sub, $sub_host) !== false) {
                        $found_user = $check;
                        $found_panel = $panel;
                        break;
                    }
    ...
    // Method 4: Try all usernames from all panels (brute-force by host)
    if ($found_user === null && !empty($candidate_usernames)) {
        foreach ($panels as $panel) {
            foreach ($candidate_usernames as $try_username) {
                $check = $ManagePanel->DataUser($panel['name_panel'], $try_username);
                if (!isset($check['username']) || $check['status'] == 'Unsuccessful') continue;
                $panel_sub = rtrim($check['subscription_url'] ?? '', '/');
                $panel_sub_host = parse_url($panel_sub, PHP_URL_HOST) ?? '';
                if ($panel_sub_host === $sub_host) {
                    $found_user = $check;
                    $found_panel = $panel;
                    break 2;
                }
    ...
    if ($found_user === null) {
        $guide = "❌ <b>سرویسی با این لینک یافت نشد.</b>\n\n📋 <b>راهنما:</b>\n• مطمئن شوید لینک ساب دقیقاً از ربات یا پنل کپی شده\n• لینک ساب معمولاً شامل نام کاربری شما است\n• لینک نباید تغییر داده شده باشد\n\n🔄 دوباره تلاش کنید یا با پشتیبانی تماس بگیرید.";
        sendmessage($from_id, $guide, null, 'html');
        step('home', $from_id);
        return;
    }

    // Check if service already exists in user's list
    $existing = $pdo->prepare("SELECT id_invoice FROM invoice WHERE id_user = ? AND username = ? AND (status='active' OR status='send_on_hold' OR status='sendedwarn')");
    $existing->execute([$from_id, $found_user['username']]);
    if ($existing->rowCount() > 0) {
        sendmessage($from_id, "⚠️ این سرویس قبلاً در لیست شما وجود دارد.", null, 'html');
        step('home', $from_id);
        return;
    }

    // Add service to invoice table
    $randomString = bin2hex(random_bytes(6));
    $notifctions = json_encode(['volume' => false, 'time' => false]);
    $data_limit_gb = $found_user['data_limit'] ? round($found_user['data_limit'] / (1024*1024*1024), 2) : 0;
    $service_time_days = $found_user['expire'] ? max(0, round(($found_user['expire'] - time()) / 86400)) : 0;
    $stmt_ins = $pdo->prepare("INSERT IGNORE INTO invoice (id_user, id_invoice, username, time_sell, Service_location, name_product, price_product, Volume, Service_time, Status, notifctions, user_info) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
    $stmt_ins->execute([
        $from_id, $randomString, $found_user['username'], time(),
        $found_panel['name_panel'], '🔗 سرویس وارد شده', '0',
        $data_limit_gb, $service_time_days, 'active', $notifctions, $sublink_input
    ]);
    $status_icons_map = ['active'=>'🟢','on_hold'=>'⚪','limited'=>'🔴','expired'=>'🔴','disabled'=>'🔴'];
    $s_icon = $status_icons_map[$found_user['status']] ?? '🟡';
    // Format expiry date
    $expire_text = $found_user['expire'] ? date('Y/m/d', $found_user['expire']) : 'نامحدود';
    $remain_text = $data_limit_gb > 0 ? round(($found_user['data_limit'] - ($found_user['used_traffic'] ?? 0)) / (1024*1024*1024), 2) . ' گیگ' : 'نامحدود';
    $success_text = "✅ <b>سرویس با موفقیت اضافه شد!</b>\n\n👤 نام کاربری: <code>{$found_user['username']}</code>\n{$s_icon} وضعیت: {$found_user['status']}\n🌍 پنل: {$found_panel['name_panel']}\n📅 تاریخ انقضا: {$expire_text}\n♾ حجم باقیمانده: {$remain_text}";
    sendmessage($from_id, $success_text, $keyboard, 'html');
    step('home', $from_id);
    ...
    if (!in_array($status, ["active", "on_hold", "disabled", "Unknown"])) {
        ...
        $status_icon_display2 = ($status_color_map2[$status] ?? '⚪') . ' ';
        $status_var_display2 = $status_icon_display2 . $status_var;
        $textinfo = sprintf($textbotlang['users']['status']['infoDetailed'], $status_var_display2, $DataUserOut['username'], $nameloc['Service_location'], $nameloc['name_product'], $lastonline, $LastTraffic, $usedTrafficGb, $RemainingVolume, $Percent, $expirationDate, $day, $nameconfig);
        ...
        if ($DataUserOut['status'] == "active") {
            $namestatus = '⏹ توقف سرویس';
        } elseif ($DataUserOut['status'] == "on_hold") {
            $namestatus = '💡 روشن کردن اکانت';
        } else {
            $namestatus = $textbotlang['users']['status']['btnTurnOn'];
        }
        // Build keyboard matching image layout
        $keyboarddate = array(
            'qrcode' => array(
                'text' => '📷 دریافت QR Code',
                'callback_data' => "qrcode_",
                'full_width' => true
            ),
            'linksub' => array(
            ...
            'opensublink' => array(
                'text' => '🌐 باز کردن لینک ساب در مرورگر',
                'url' => (!empty($DataUserOut['subscription_url'])) ? $DataUserOut['subscription_url'] : null,
                'full_width' => true
            ),
            'extend' => array(
                'text' => '♻️ ' . $textbotlang['users']['extend']['title'],
            ...
            'updateinfo' => array(
                'text' => '📶 وضعیت و راهنمای سرویس ↗',
                'url' => (!empty($DataUserOut['subscription_url'])) ? $DataUserOut['subscription_url'] : null,
                'full_width' => true
            ),
            'removeservice' => array(
                'text' => '🗑 درخواست حذف سرویس و بازگشت وجه',
                'callback_data' => "removeserviceuser_",
                'full_width' => true
            ),
            'changelink' => array(
                'text' => '❌ حذف لینک فعلی سرویس و دریافت لینک جدید',
        ...
        if ($marzban['type'] == "ibsng" || $marzban['type'] == "mikrotik") {
            ...
            unset($keyboarddate['opensublink']);
            unset($keyboarddate['updateinfo']);
        ...
        if ($marzbanstatusextra == "offextra")
            unset($keyboarddate['Extra_volume']);
        // Remove URL buttons if subscription_url is not available
        if (empty($DataUserOut['subscription_url'])) {
            unset($keyboarddate['opensublink']);
            unset($keyboarddate['updateinfo']);
        }
        $tempArray = [];
        ...
        foreach ($keyboarddate as $keyboardtext) {
            $is_full = isset($keyboardtext['full_width']) && $keyboardtext['full_width'];
            // Build button: URL button or callback button
            // Skip buttons with null/empty URL (means feature unavailable)
            if (array_key_exists('url', $keyboardtext)) {
                if (empty($keyboardtext['url'])) continue; // skip invalid URL buttons
                $btn = ['text' => $keyboardtext['text'], 'url' => $keyboardtext['url']];
            } else {
                $btn = ['text' => $keyboardtext['text'], 'callback_data' => $keyboardtext['callback_data'] . $username];
            }
            if ($is_full) {
                // Flush existing pair first
                if (count($tempArray) > 0) {
                    $keyboardsetting['inline_keyboard'][] = $tempArray;
                    $tempArray = [];
                }
                // Add full-width button on its own row
                $keyboardsetting['inline_keyboard'][] = [$btn];
            } else {
                $tempArray[] = $btn;
        ...
        $status_icon_display = ($status_color_map[$status] ?? '⚪') . ' ';
        $status_var_display = $status_icon_display . $status_var;
        $textinfo = sprintf($textbotlang['users']['status']['infoFull'], $status_var_display, $DataUserOut['username'], $userpassword, $nameconfig, $nameloc['Service_location'], $nameloc['name_product'], $LastTraffic, $usedTrafficGb, $RemainingVolume, $Percent, $expirationDate, $day, $textconnect);
    }
    if ($user['step'] == "getuseragnetservice") {
        sendmessage($from_id, $textinfo, $keyboardsetting, 'html');
    } elseif ($datain == "productcheckdata") {
    ...
    step('home', $from_id);
    return;
} elseif (preg_match('/^qrcode_(\w+)/', $datain, $dataget)) {
    // QR Code handler - send QR of subscription URL + all configs QR
    $qr_id_invoice = $dataget[1];
    $nameloc = select("invoice", "*", "id_invoice", $qr_id_invoice, "select");
    ...
    if (empty($subscriptionurl)) {
        sendmessage($from_id, "❌ لینک اشتراک برای این سرویس در دسترس نیست.", null, 'html');
        return;
    }
    $bakinfos = json_encode([
        'inline_keyboard' => [
            [
                ['text' => '🌐 باز کردن لینک ساب در مرورگر', 'url' => $subscriptionurl],
            ],
            [
                ['text' => $textbotlang['users']['status']['backinfo'], 'callback_data' => "product_" . $qr_id_invoice],
            ]
        ]
    ]);
    // Send QR of subscription URL first
    $randomString = bin2hex(random_bytes(3));
    $urlimage = qrTempPath("$from_id$randomString.png");
    $qrCode = createqrcode($subscriptionurl);
    $caption = "📷 <b>QR Code سرویس شما</b>\n\n👤 <code>{$nameloc['username']}</code>\n🔗 <code>$subscriptionurl</code>";
    $first_kb = empty($links) ? $bakinfos : null;
    if ($qrCode !== null && @file_put_contents($urlimage, $qrCode->getString()) !== false) {
        ...
        sendmessage($from_id, $caption, $first_kb, 'HTML');
    }
    // Send each config as a QR image with its name
    if (!empty($links)) {
        $total = count($links);
        for ($i = 0; $i < $total; $i++) {
            $link = $links[$i];
            ...
            if (strpos($link, '#') !== false) {
                $config_name = urldecode(substr($link, strrpos($link, '#') + 1));
            }
            if (empty($config_name)) {
                $proto = explode('://', $link)[0] ?? '';
                $config_name = strtoupper($proto) . " " . ($i + 1);
            }
            $num = $i + 1;
            $caption2 = "📷 <b>QR کانفیگ {$num} از {$total}</b>\n📌 <b>{$config_name}</b>\n\n<code>{$link}</code>";
            $randomString2 = bin2hex(random_bytes(3));
            $urlimage2 = qrTempPath("$from_id$randomString2.png");
            $qrCode2 = createqrcode($link);
            $kb = ($i == $total - 1) ? $bakinfos : null;
            if ($qrCode2 !== null && @file_put_contents($urlimage2, $qrCode2->getString()) !== false) {
                addBackgroundImage($urlimage2, $qrCode2, 'images.jpg');
                telegram('sendphoto', [
                    'chat_id' => $from_id,
                    'photo' => new CURLFile($urlimage2),
                    'caption' => $caption2,
                    'reply_markup' => $kb,
                    'parse_mode' => "HTML",
                ]);
                ...
                sendmessage($from_id, $caption2, $kb, 'HTML');
            }
        }
    }
...
<code>$subscriptionurl</code>";
    ...
    // Direct delivery: every config is sent separately with its name.
    // No config keyboard is generated.
    telegram('answerCallbackQuery', [
        'callback_query_id' => $callback_query_id,
        'text' => 'در حال ارسال کانفیگ‌ها…',
        'show_alert' => false,
        'cache_time' => 1,
    ]);
    Editmessagetext($from_id, $message_id, '📤 <b>کانفیگ‌های سرویس در حال ارسال هستند…</b>', null);
    sendConfigsDirectly($from_id, $DataUserOut['links'], $nameloc['id_invoice'], $nameloc['username'], true);
 } elseif (preg_match('/^configget_(.+)_(\w+)$/', $datain, $dataget)) {
    // Backward compatibility for old messages/buttons. Even old callbacks now
    // trigger direct delivery instead of rebuilding the old config keyboard.
    $id_invoice = $dataget[1];
    $nameloc = select("invoice", "*", "id_invoice", $id_invoice, "select");
    ...
    telegram('answerCallbackQuery', [
        'callback_query_id' => $callback_query_id,
        'text' => 'کانفیگ‌ها به‌صورت جداگانه ارسال می‌شوند.',
        'show_alert' => false,
        'cache_time' => 1,
    ]);
    ...
    if ($marzban_list_get['config'] == "onconfig") {
        Editmessagetext($from_id, $message_id, $textconfig, null);
        sendConfigsDirectly($from_id, $DataUserOut['configs'], $nameloc['id_invoice'], $nameloc['username'], true);
    } else {
...
if (in_array($from_id, $admin_ids)) {
    // adminrulecheck is also reused by several report/notification loops above.
    // Refresh it immediately before entering the admin dispatcher so the
    // current click is authorized against the CURRENT admin, not the last
    // administrator visited by one of those loops.
