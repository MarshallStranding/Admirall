...
if ($text == "/start" || $datain == "start" || $text == "start") {
    ...
    if (!in_array($status, ["active", "on_hold", "disabled", "Unknown"])) {
        ...
        if ($marzbanstatusextra == "offextra")
            unset($keyboarddate['Extra_volume']);
        // ========================================================
        // کیبورد مدیریت سرویس - چیدمان دقیق مطابق طراحی
        // ========================================================
        $keyboardsetting = ['inline_keyboard' => []];

        // ردیف ۱: دریافت QR Code (تمام عرض)
        if (isset($keyboarddate['config'])) {
            $keyboardsetting['inline_keyboard'][] = [
                ['text' => '📷 دریافت QR Code', 'callback_data' => 'configget_' . $username . '_1520']
            ];
        }

        // ردیف ۲: دریافت لینک اشتراک + دریافت کانفیگ‌ها
        $row2 = [];
        if (isset($keyboarddate['linksub'])) {
            $row2[] = ['text' => $keyboarddate['linksub']['text'], 'callback_data' => $keyboarddate['linksub']['callback_data'] . $username];
        }
        if (isset($keyboarddate['config'])) {
            $row2[] = ['text' => $keyboarddate['config']['text'], 'callback_data' => $keyboarddate['config']['callback_data'] . $username];
        }
        if (!empty($row2)) {
            $keyboardsetting['inline_keyboard'][] = $row2;
        }

        // ردیف ۳: توقف سرویس + تمدید سرویس
        $row3 = [];
        if (isset($keyboarddate['changestatus'])) {
            $row3[] = ['text' => $keyboarddate['changestatus']['text'], 'callback_data' => $keyboarddate['changestatus']['callback_data'] . $username];
        }
        if (isset($keyboarddate['extend'])) {
            $row3[] = ['text' => $keyboarddate['extend']['text'], 'callback_data' => $keyboarddate['extend']['callback_data'] . $username];
        }
        if (!empty($row3)) {
            $keyboardsetting['inline_keyboard'][] = $row3;
        }

        // ردیف ۴: وضعیت و راهنمایی سرویس (url - باز کردن لینک ساب در مرورگر)
        if (!empty($DataUserOut['subscription_url']) && isset($marzban['sublink']) && $marzban['sublink'] === 'onsublink') {
            $keyboardsetting['inline_keyboard'][] = [
                ['text' => '📶 وضعیت و راهنمایی سرویس', 'url' => $DataUserOut['subscription_url']]
            ];
        } elseif (isset($keyboarddate['ekhtelal'])) {
            $keyboardsetting['inline_keyboard'][] = [
                ['text' => $keyboarddate['ekhtelal']['text'], 'callback_data' => $keyboarddate['ekhtelal']['callback_data'] . $username]
            ];
        }

        // ردیف ۵: درخواست حذف سرویس و بازگشت وجه (تمام عرض)
        if (isset($keyboarddate['removeservice'])) {
            $keyboardsetting['inline_keyboard'][] = [
                ['text' => $keyboarddate['removeservice']['text'], 'callback_data' => $keyboarddate['removeservice']['callback_data'] . $username]
            ];
        }

        // ردیف ۶: حذف لینک فعلی سرویس و دریافت لینک جدید (تمام عرض)
        if (isset($keyboarddate['changelink'])) {
            $keyboardsetting['inline_keyboard'][] = [
                ['text' => '❌ ' . $keyboarddate['changelink']['text'], 'callback_data' => $keyboarddate['changelink']['callback_data'] . $username]
            ];
        }

        // دکمه‌های اضافی: Extra volume, Extra time, transfer, change-location, changenameconfig
        $extraRow = [];
        foreach (['Extra_volume', 'Extra_time'] as $k) {
            if (isset($keyboarddate[$k])) {
                $extraRow[] = ['text' => $keyboarddate[$k]['text'], 'callback_data' => $keyboarddate[$k]['callback_data'] . $username];
            }
        }
        if (!empty($extraRow)) {
            $keyboardsetting['inline_keyboard'][] = $extraRow;
        }
        foreach (['transfor', 'change-location', 'changenameconfig', 'updateinfo'] as $k) {
            if (isset($keyboarddate[$k])) {
                $keyboardsetting['inline_keyboard'][] = [
                    ['text' => $keyboarddate[$k]['text'], 'callback_data' => $keyboarddate[$k]['callback_data'] . $username]
                ];
            }
        }

        // ردیف آخر: بازگشت به لیست سرویس‌ها
    ...
    if ($marzban_list_get['type'] == "WGDashboard") {
        ...
        $safeSubUrl = htmlspecialchars($subscriptionurl, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
        $textsub = "🔗 <b>لینک اشتراک سرویس</b>\n\n<code>{$safeSubUrl}</code>";
        $bakinfos = json_encode([
            'inline_keyboard' => [
                [
    ...
    // ارسال کانفیگ‌ها یکی‌یکی با اسم
    deletemessage($from_id, $message_id);
    ...
    if ($dataget[2] == "1520") {
        // ارسال همه کانفیگ‌ها به‌صورت QR Code یکی یکی با اسم
        $total = count($DataUserOut['links']);
        for ($i = 0; $i < $total; ++$i) {
            $configLink = $DataUserOut['links'][$i];
            $configName = extract_config_name($configLink, $i + 1);
            $safeConfigName = htmlspecialchars($configName, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
            $safeConfigLink = htmlspecialchars($configLink, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
            $captionQr = "<b>{$safeConfigName}</b>\n\n<code>{$safeConfigLink}</code>";

            $randomString = bin2hex(random_bytes(3));
            $urlimage = qrTempPath("$from_id$randomString.png");
            $qrCode = createqrcode($configLink);
            // دکمه بازگشت فقط برای آخرین کانفیگ
            $replyMarkup = ($i === $total - 1) ? $backToService : null;
            if ($qrCode === null || @file_put_contents($urlimage, $qrCode->getString()) === false) {
                sendmessage($from_id, $captionQr, $replyMarkup, 'HTML');
                continue;
            }
            addBackgroundImage($urlimage, $qrCode, 'images.jpg');
            telegram('sendphoto', [
            ...
            @unlink($urlimage);
            if ($i < $total - 1) {
                usleep(200000); // anti-flood
            }
        }
        return;
    }
    // ارسال تک کانفیگ با QR
    $singleLink = $DataUserOut['links'][$dataget[2]];
    $singleName = extract_config_name($singleLink, intval($dataget[2]) + 1);
    $safeSingleName = htmlspecialchars($singleName, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
    $safeSingleLink = htmlspecialchars($singleLink, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
    $captionSingle = "<b>{$safeSingleName}</b>\n\n<code>{$safeSingleLink}</code>";
    $randomString = bin2hex(random_bytes(3));
    $urlimage = qrTempPath("$from_id$randomString.png");
    $qrCode = createqrcode($singleLink);
    if ($qrCode === null || @file_put_contents($urlimage, $qrCode->getString()) === false) {
        sendmessage($from_id, $captionSingle, $backToService, 'HTML');
        return;
    }
    addBackgroundImage($urlimage, $qrCode, 'images.jpg');
    telegram('sendphoto', [
        'chat_id' => $from_id,
        'photo' => new CURLFile($urlimage),
        'caption' => $captionSingle,
        'parse_mode' => "HTML",
        'reply_markup' => $backToService,
    ]);
    @unlink($urlimage);
    ...
    if ($split_data[0] == "getconfigafterpay") {
        ...
        $Confirm_pay = json_encode([
            'inline_keyboard' => [
                ...
                [
                    ['text' => $textbotlang['keyboard']['configInfo'] ?? '📋 اطلاعات سرویس', 'callback_data' => "manageinvoice_{$get_invoice['id_invoice']}"],
                ]
            ]
        ]);
        $textdiscount = "";
