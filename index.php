...
<code>$subscriptionurl</code>";
    ...
    $DataUserOut = $ManagePanel->DataUser($nameloc['Service_location'], $nameloc['username']);
    // Back button goes to the correct product page
    $bakinfos = json_encode([
        'inline_keyboard' => [
            [
    ...
    // Helper: extract config name from link
    $getConfigName = function($link, $idx) {
        if (strpos($link, '#') !== false) {
            $name = urldecode(substr($link, strrpos($link, '#') + 1));
            if (!empty($name)) return $name;
        }
        $proto = explode('://', $link)[0] ?? '';
        return strtoupper($proto) . " " . ($idx + 1);
    };
    if ($dataget[2] == "all") {
        // Send ALL configs as QR images with names
        $total = count($links);
        for ($i = 0; $i < $total; $i++) {
            $link = $links[$i];
            $config_name = $getConfigName($link, $i);
            $num = $i + 1;
            $caption = "📷 <b>QR کانفیگ {$num} از {$total}</b>\n📌 <b>{$config_name}</b>\n\n<code>{$link}</code>";
            $randomString = bin2hex(random_bytes(3));
            $urlimage = qrTempPath("$from_id$randomString.png");
            $qrCode = createqrcode($link);
            // Last one gets the back button
            $kb = ($i == $total - 1) ? $bakinfos : null;
            if ($qrCode !== null && @file_put_contents($urlimage, $qrCode->getString()) !== false) {
                addBackgroundImage($urlimage, $qrCode, 'images.jpg');
                telegram('sendphoto', [
                ...
                sendmessage($from_id, $caption, $kb, 'HTML');
            }
        }
        return;
    }
    // Single config QR by index
    $idx = intval($dataget[2]);
    if (!isset($links[$idx])) {
        sendmessage($from_id, $textbotlang['users']['status']['configReadError'], null, 'html');
        return;
    }
    $link = $links[$idx];
    $config_name = $getConfigName($link, $idx);
    $caption = "📷 <b>QR کانفیگ</b>\n📌 <b>{$config_name}</b>\n\n<code>{$link}</code>";
    $randomString = bin2hex(random_bytes(3));
    $urlimage = qrTempPath("$from_id$randomString.png");
    $qrCode = createqrcode($link);
    if ($qrCode === null || @file_put_contents($urlimage, $qrCode->getString()) === false) {
        sendmessage($from_id, $caption, $bakinfos, 'HTML');
        return;
    }
    addBackgroundImage($urlimage, $qrCode, 'images.jpg');
    telegram('sendphoto', [
