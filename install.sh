...
INSTALL_LOG="/tmp/admirall_install.log"
...
_step_eta() {
    case "$1" in
        ...
        "Downloading Admirallbot"*)                echo 20 ;;
...
banner()  {
    echo
    _drule
    printf "  ${C_OK}▌${CR} ${C_TITLE}Admirallbot${CR}  ${C_DIM}— VPN Subscription Management${CR}\n"
    _drule
}
...
ensure_dns() {
    ...
    elif [ -f "$RESOLV" ] && [ ! -f "${RESOLV}.admirall.bak" ]; then
        cp -a "$RESOLV" "${RESOLV}.admirall.bak" 2>/dev/null
    fi
...
# Ensure /usr/local/bin/admirallbot points at the master script
_link_admirallbot() {
...
function self_update_script() {
    ...
    if [ "$valid" -ne 1 ]; then
        ...
        _link_admirallbot "$MASTER_PATH" "$BIN_LINK"
        return 0
    fi
...
# ── Repo / paths ─────────────────────────────────────────────
BOT_DIR_DEFAULT="/var/www/html/admirallbotconfig"
CONFIG_FILE_DEFAULT="$BOT_DIR_DEFAULT/config.php"
GIT_REPO="MarshallStranding/Admirall"
LATEST_CACHE="/tmp/.admirall_latest_version"
IP_CACHE="/tmp/.admirall_server_ip"
...
STATE_DIR="/root/confadmirall"
STATE_FILE="$STATE_DIR/.admirall_install_state"
...
php_repo_disable() {
    ...
    for f in /etc/apt/sources.list.d/*ondrej*php*.sources /etc/apt/sources.list.d/*ondrej*php*.list; do
        [ -f "$f" ] || continue
        mv -f "$f" "$f.disabled-by-admirall" && n=$((n + 1))
    done
    [ "$n" -gt 0 ]
}
...
setup_mysql_root() {
    sudo mkdir -p /root/confadmirall || return 1
    sudo chmod 700 /root/confadmirall || return 1
    touch /root/confadmirall/dbrootadmirall.txt || return 1
    sudo chmod 600 /root/confadmirall/dbrootadmirall.txt || return 1
    ...
    echo "\$user = 'root';"               >> /root/confadmirall/dbrootadmirall.txt
    echo "\$pass = '${randomdbpasstxt}';" >> /root/confadmirall/dbrootadmirall.txt
    echo "\$path = '${RANDOM_NUMBER}';"   >> /root/confadmirall/dbrootadmirall.txt
    passs=$(grep '$pass' /root/confadmirall/dbrootadmirall.txt | cut -d"'" -f2)
    userrr=$(grep '$user' /root/confadmirall/dbrootadmirall.txt | cut -d"'" -f2)
    local alter_ok=0
    ...
    local dropin="$dropin_dir/zz-admirall-recovery.cnf"
...
# Only runs on a brand-new install (never on resume / Admirallbot's own partial state).
...
install_pause() {
    ...
    echo -e "      ${C_KEY}admirallbot install${CR}"
...
version_section() {
    ...
    _kv "Channel" "${C_DIM}t.me/admirallbot${CR}"
    _kv "Group" "${C_DIM}t.me/admirallbotgroup${CR}"
}
...
function renew_ssl() {
    ...
    local cfg="/var/www/html/admirallbotconfig/config.php"
    local domain=""
    if [ -f "$cfg" ]; then
    ...
    if ! command -v certbot >/dev/null 2>&1; then
        echo -e "  ${C_BAD}●${CR} ${C_BAD}certbot is not installed. Install Admirallbot first.${CR}"
        sleep 1; show_menu; return 1
    fi
...
function backup_bot() {
    ...
    CONFIG_PATH="/var/www/html/admirallbotconfig/config.php"
    if [ ! -f "$CONFIG_PATH" ]; then
        printf "    ${C_BAD}●${CR} ${C_BAD}Admirallbot is not installed. config.php not found.${CR}\n"
        echo ""
        printf "  ${C_PROMPT}❯${CR} Press Enter to return to the menu... "
        read -r _
        show_menu
    ...
    local backup_file="/root/admirall_backup_${backup_date}.sql"
    ...
    if [ -n "$bot_token" ] && [ -n "$admin_id" ]; then
        ...
        send_result=$(curl -s -o /dev/null -w "%{http_code}" \
            ...
            -F "caption=📦 Admirallbot DB Backup (${backup_date})" \
...
function import_bot() {
    ...
    while IFS= read -r f; do
        ...
        i=$((i + 1))
    done < <(find /root -maxdepth 1 -name 'admirall_backup_*.sql' -type f 2>/dev/null | sort -r)
...
function show_menu() {
    show_logo
    _sec "Menu"
    _mi "1" "Install Admirallbot"
    _mi "2" "Update Admirallbot"
    _mi "3" "Remove Admirallbot"
...
function show_help_screen() {
    ...
    _sec "Commands"
    _kv "install" "${C_DIM}Install Admirallbot${CR}"
    _kv "update" "${C_DIM}Update Admirallbot (choose channel / version)${CR}"
    _kv "remove" "${C_DIM}Remove Admirallbot and its services${CR}"
    ...
    _sec "Examples"
    printf "    ${C_KEY}admirallbot install --channel auto${CR}\n"
    printf "    ${C_KEY}admirallbot install --token 123:ABC \\\\${CR}\n"
    printf "    ${C_DIM}            --admin 111 --domain bot.example.com --version 0.1.7${CR}\n"
    printf "    ${C_KEY}admirallbot update --version 0.1.6${CR}\n"
    printf "    ${C_KEY}admirallbot update --channel release${CR}\n"
    printf "    ${C_KEY}admirallbot remove${CR}\n"
    printf "    ${C_KEY}admirallbot backup${CR}\n"
    printf "    ${C_KEY}admirallbot import${CR}\n"
...
function fix_update_issues() {
    ...
    [ -f "$target" ] && cp "$target" "$target.admirallbackup"

    local parked=""
    if [ "$fmt" = "deb822" ] && [ -s "$LEGACY" ]; then
        cp "$LEGACY" "$LEGACY.admirallbackup" && : > "$LEGACY" && parked="$LEGACY"
    fi
...
EOF
        ...
        if apt-get update --allow-releaseinfo-change 2>/dev/null; then
            ...
            rm -f "$target.admirallbackup"
            [ -n "$parked" ] && rm -f "$parked.admirallbackup"
            return 0
        fi
    done
    if [ -f "$target.admirallbackup" ]; then
        mv "$target.admirallbackup" "$target"
    else
        rm -f "$target"
    fi
    [ -n "$parked" ] && [ -f "$parked.admirallbackup" ] && mv "$parked.admirallbackup" "$parked"
...
function install_bot() {
    BOT_DIR="/var/www/html/admirallbotconfig"
    ...
    if [ -f "$CONFIG_FILE_DEFAULT" ] && ! has_resumable_state; then
        ...
        printf "    ${C_BAD}●${CR} ${C_BAD}Admirallbot is already installed on this server.${CR}\n"
    ...
    if ! phase_done FILES; then
        ...
        TEMP_DIR="/tmp/admirallbot"
        rm -rf "$TEMP_DIR"; mkdir -p "$TEMP_DIR"
        run_step "Downloading Admirallbot (${SRC_LABEL_RESUME})" "wget -O '$TEMP_DIR/bot.zip' '$ZIP_URL'" \
    ...
    if ! phase_done DBROOT; then
        if [ ! -f "/root/confadmirall/dbrootadmirall.txt" ] || ! grep -q '\$pass' /root/confadmirall/dbrootadmirall.txt 2>/dev/null; then
    ...
    PATHS=$(cat /root/confadmirall/dbrootadmirall.txt | grep '$path' | cut -d"'" -f2)
...
EOF
    ...
    ROOT_PASSWORD=$(cat /root/confadmirall/dbrootadmirall.txt | grep '$pass' | cut -d"'" -f2)
    ROOT_USER="root"
    ...
    if ! phase_done CONFIG; then
        wait
        sleep 1
        file_path="/var/www/html/admirallbotconfig/config.php"
        if [ -f "$file_path" ]; then
            rm "$file_path" || {
        ...
        cat <<EOF > /var/www/html/admirallbotconfig/config.php
<?php
...
?>
EOF
        sudo chown www-data:www-data /var/www/html/admirallbotconfig/config.php 2>/dev/null
        sudo chmod 640 /var/www/html/admirallbotconfig/config.php 2>/dev/null
        mark_phase CONFIG
    else
        secrettoken="$(state_get SECRET)"
    ...
    if ! phase_done WEBHOOK; then
        ...
        MESSAGE="✅ The Admirallbot is installed! for start the bot send /start command."
    ...
    printf "    ${C_OK}●${CR} ${C_OK}Admirallbot is installed and the webhook is set.${CR}\n"
    ...
    _sec "Manage"
    _kv "Command" "${C_DIM}run ${CR}${C_KEY}admirallbot${CR}${C_DIM} anytime to open this panel${CR}"
    echo ""
    _rule
    echo ""

    chmod +x /root/install.sh
    ln -sf /root/install.sh /usr/local/bin/admirallbot
    self_update_script
}
function update_bot() {
    clear
    ...
    if [ ! -d "$BOT_DIR" ]; then
        _sec "Update"
        printf "    ${C_BAD}●${CR} ${C_BAD}Admirallbot is not installed. Install it first.${CR}\n"
        sleep 2
        show_menu
        return 1
    fi
    ...
    print_header "Updating Admirallbot"
    ...
    TEMP_DIR="/tmp/admirallbot_update"
    rm -rf "$TEMP_DIR"; mkdir -p "$TEMP_DIR"
    ...
    TEMP_CONFIG="/root/admirall_config_backup.php"
    if [ -f "$CONFIG_PATH" ]; then
...
EOF
    ...
    rm -rf "$TEMP_DIR"
    echo -e "\n\e[92mAdmirallbot updated to latest version successfully!\033[0m"
    if [ -f "/root/install.sh" ]; then
        sudo chmod +x /root/install.sh
        sudo ln -sf /root/install.sh /usr/local/bin/admirallbot
        echo -e "\e[92mEnsured /root/install.sh is executable and 'admirallbot' command is linked.\033[0m"
    else
        echo -e "\e[91mError: /root/install.sh not found after update attempt.\033[0m"
    fi
}
function remove_bot() {
    echo -e "\e[33mStarting Admirallbot removal process...\033[0m"
    LOG_FILE="/var/log/remove_bot.log"
    echo "Log file: $LOG_FILE" > "$LOG_FILE"
    BOT_DIR="/var/www/html/admirallbotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[31m[ERROR]\033[0m Admirallbot is not installed (/var/www/html/admirallbotconfig not found)." | tee -a "$LOG_FILE"
        echo -e "\e[33mNothing to remove. Exiting...\033[0m" | tee -a "$LOG_FILE"
        sleep 2
        exit 1
    fi
    read -p "Are you sure you want to remove Admirallbot and its dependencies? (y/n): " choice
    if [[ ! "$choice" =~ ^[Yy]$ ]]; then
        echo "Aborting..." | tee -a "$LOG_FILE"
        exit 0
    fi
    echo "Removing Admirallbot..." | tee -a "$LOG_FILE"
    if command -v crontab >/dev/null 2>&1 || [ -x /usr/bin/crontab ]; then
        ...
        echo -e "\e[92mRemoved Admirallbot cron jobs.\033[0m" | tee -a "$LOG_FILE"
    fi
    CONFIG_PATH="/var/www/html/admirallbotconfig/config.php"
    if [ -f "$CONFIG_PATH" ]; then
    ...
    # Clear Admirallbot install state so a fresh install is allowed afterwards
    sudo rm -rf /root/confadmirall
    echo -e "\e[92mAdmirallbot, MySQL, and their dependencies have been completely removed.\033[0m" | tee -a "$LOG_FILE"
}
...
function migrate_to_pro() {
    ...
    OLD_BOT_DIR="/var/www/html/admirallbotconfig"
    ...
    BACKUP_FILE="/root/admirallbot_backup.sql"
    if [ ! -f "$BACKUP_FILE" ]; then
        echo -e "\033[31m[ERROR] Backup file not found at $BACKUP_FILE\033[0m"
        echo -e "\033[33mPlease run the 'admirallbot' command (Free Version Script) and use option 4 to create a backup.\033[0m"
        exit 1
    else
    ...
    echo -e "\033[33mOnly the main bot (admirallbotconfig) will be migrated.\033[0m"
    echo -e "\033[33mExisting Additional Bots in /var/www/html/ might stop working.\033[0m"
    echo -e "\033[36mFound directories:\033[0m"
    ls -d /var/www/html/*/ 2>/dev/null | grep -v "admirallbotconfig"
    echo ""
    ...
    ROOT_CRED_FILE="/root/confadmirall/dbrootadmirall.txt"
    ROOT_PASS=""
    ROOT_USER="root"
    ...
    OLD_CONFIG="/var/www/html/admirallbotconfig/config.php"
    ...
    NEW_BOT_DIR="/var/www/html/admirallbotconfig"
    rm -rf "$OLD_BOT_DIR"
    mkdir -p "$NEW_BOT_DIR"
    ZIP_URL="https://github.com/MarshallStranding/Admirall/archive/refs/heads/main.zip"
    TEMP_DIR="/tmp/admirallbot_mig"
    mkdir -p "$TEMP_DIR"
    run_step "Downloading Admirallbot source" "wget -q -O '$TEMP_DIR/bot.zip' '$ZIP_URL'" \
        || { show_step_error; echo -e "\033[31mError: Failed to download Admirallbot source.\033[0m"; exit 1; }
...
EOF
    ...
    chmod +x /root/install.sh
    rm -f /usr/local/bin/admirallbot
    ln -sf /root/install.sh /usr/local/bin/admirallbot
    clear
    ...
    echo -e "\033[33mUse command 'admirallbot' to manage the bot from now on.\033[0m"
    echo ""
}
...
print_usage() {
  ...
  Admirallbot - management script

  Usage:
    admirallbot [command] [options]

  Commands:
    install            Install Admirallbot
    update             Update Admirallbot
    remove             Remove Admirallbot
  ...
  Examples:
    admirallbot install --channel auto
    admirallbot install --token 123:ABC --admin 111 --domain bot.example.com --version 0.1.7
    admirallbot update --channel release
    admirallbot update --version 0.1.6
