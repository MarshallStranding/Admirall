...
// Re-read the current administrator record here. index.php uses the same
// variable name for notification/report loops, so by the time admin.php is
// included it may otherwise contain the LAST admin in the loop instead of
// the admin who actually clicked the keyboard button.
...
/**
 * Build one self-contained panel status report for the administrator.
 * This intentionally reuses the panel-specific connection functions already
 * present in Admirall instead of introducing a second panel API layer.
 */
function admirall_admin_panel_status_report(array $panel, array $textbotlang, PDO $pdo): string
{
    ...
    $statusText = $textbotlang['keyboard']['adminVideo']['panelConnected'] ?? '🟢 Connected';
    $connectionOk = false;
    $stats = [];
    ...
    try {
        switch ($panel['type'] ?? '') {
            case 'marzban':
                $token = token_panel($panel['code_panel'], false);
                if (isset($token['access_token'])) {
                    $raw = Get_System_Stats($panel['name_panel']);
                    if (is_array($raw) && empty($raw['error']) && (empty($raw['status']) || $raw['status'] < 400)) {
                        $stats = isset($raw['body']) && is_string($raw['body']) ? (json_decode($raw['body'], true) ?: []) : $raw;
                        $connectionOk = true;
                    }
                }
                break;
            case 'marzneshin':
                ...
                if (isset($token['access_token'])) {
                    $raw = Get_System_Statsm($panel['name_panel']);
                    if (is_array($raw) && empty($raw['error']) && (empty($raw['status']) || $raw['status'] < 400)) {
                        $stats = isset($raw['body']) && is_string($raw['body']) ? (json_decode($raw['body'], true) ?: []) : $raw;
                        $connectionOk = true;
                    }
                ...
                break;
            case 'x-ui_single':
                $raw = status_server_xui($panel);
                if (is_array($raw) && isset($raw['body'])) {
                    $stats = json_decode($raw['body'], true) ?: [];
                    $connectionOk = empty($raw['error']) && (empty($raw['status']) || $raw['status'] == 200) && !empty($stats['obj']);
                }
                break;
            case 'alireza_single':
                $raw = login($panel['code_panel'], false);
                $connectionOk = is_array($raw) && !empty($raw['success']);
                break;
            case 'hiddify':
                $raw = serverstatus($panel['name_panel']);
                if (is_array($raw) && isset($raw['body'])) {
                    $stats = json_decode($raw['body'], true) ?: [];
                    $connectionOk = empty($raw['error']) && (empty($raw['status']) || $raw['status'] < 400) && isset($stats['stats']);
                }
                break;
            case 'rebecca':
                $raw = Get_System_Stats_rebecca($panel['name_panel']);
                $connectionOk = is_array($raw) && empty($raw['error']) && (empty($raw['status']) || $raw['status'] < 400);
                if ($connectionOk && isset($raw['body'])) {
                    $stats = json_decode($raw['body'], true) ?: [];
                }
                break;
            case 'mikrotik':
                $raw = login_mikrotik($panel['url_panel'], $panel['username_panel'], $panel['password_panel']);
                $connectionOk = is_array($raw) && !isset($raw['error']);
                $stats = $raw ?: [];
                break;
            case 'ibsng':
                $raw = loginIBsng($panel['url_panel'], $panel['username_panel'], $panel['password_panel']);
                $connectionOk = is_array($raw) && empty($raw['error']);
                break;
            default:
                // These panel types do not expose a single common health API in
                // the current project. We still report their local configuration
                // and database service statistics without inventing connectivity.
                $connectionOk = false;
                break;
        }
    } catch (Throwable $e) {
        error_log('Admin panel status check failed for ' . ($panel['name_panel'] ?? 'unknown') . ': ' . $e->getMessage());
        $connectionOk = false;
    }

    if (!$connectionOk) {
        $statusText = $textbotlang['keyboard']['adminVideo']['panelDisconnected'] ?? '🔴 Disconnected';
    }

    $testName = $textbotlang['common']['labels']['testServiceName'];
    $serviceStmt = $pdo->prepare("SELECT COUNT(*) FROM invoice WHERE (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn' OR status = 'send_on_hold') AND Service_location = :panel AND name_product != :test");
    $serviceStmt->execute([':panel' => $panel['name_panel'], ':test' => $testName]);
    $services = (int)$serviceStmt->fetchColumn();
    $salesStmt = $pdo->prepare("SELECT COALESCE(SUM(price_product),0) FROM invoice WHERE (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn' OR status = 'send_on_hold') AND Service_location = :panel AND name_product != :test");
    $salesStmt->execute([':panel' => $panel['name_panel'], ':test' => $testName]);
    $sales = number_format((float)$salesStmt->fetchColumn(), 0);

    $totalUsers = $stats['total_user'] ?? $stats['total'] ?? null;
    $activeUsers = $stats['active_users'] ?? $stats['users_active'] ?? $stats['active'] ?? $stats['online_users'] ?? null;
    $panelVersion = $stats['version'] ?? $stats['panelVersion'] ?? null;
    $memory = null;
    if (isset($stats['mem_total'], $stats['mem_used'])) {
        $memory = formatBytes($stats['mem_used']) . ' / ' . formatBytes($stats['mem_total']);
    } elseif (isset($stats['stats']['system']['ram_used'], $stats['stats']['system']['ram_total'])) {
        $memory = formatBytes($stats['stats']['system']['ram_used']) . ' / ' . formatBytes($stats['stats']['system']['ram_total']);
    } elseif (isset($stats['total-memory'], $stats['free-memory'])) {
        $used = max(0, (float)$stats['total-memory'] - (float)$stats['free-memory']);
        $memory = formatBytes($used) . ' / ' . formatBytes($stats['total-memory']);
    }
    $bandwidth = null;
    if (isset($stats['outgoing_bandwidth'], $stats['incoming_bandwidth'])) {
        $bandwidth = formatBytes((float)$stats['outgoing_bandwidth'] + (float)$stats['incoming_bandwidth']);
    }

    $text = ($textbotlang['keyboard']['adminVideo']['panelStatusHeader'] ?? '🖥 Panel Status') . "\n";
    $text .= "━━━━━━━━━━━━━━\n";
    $text .= ($textbotlang['keyboard']['adminVideo']['panelNameLabel'] ?? '📌 Name') . ": <b>{$name}</b>\n";
    $text .= ($textbotlang['keyboard']['adminVideo']['panelTypeLabel'] ?? '🧩 Type') . ": {$type}\n";
    $text .= ($textbotlang['keyboard']['adminVideo']['panelConnectionLabel'] ?? '🔌 Connection') . ": {$statusText}\n";
    if ($totalUsers !== null) {
        $text .= ($textbotlang['keyboard']['adminVideo']['panelTotalUsers'] ?? '👥 Users') . ": " . number_format((int)$totalUsers) . "\n";
    }
    if ($activeUsers !== null) {
        $text .= ($textbotlang['keyboard']['adminVideo']['panelActiveUsers'] ?? '🟢 Active Users') . ": " . number_format((int)$activeUsers) . "\n";
    }
    if ($panelVersion !== null) {
        $text .= ($textbotlang['keyboard']['adminVideo']['panelVersionLabel'] ?? '📦 Version') . ": <code>" . htmlspecialchars((string)$panelVersion, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "</code>\n";
    }
    if ($memory !== null) {
        $text .= ($textbotlang['keyboard']['adminVideo']['panelMemoryLabel'] ?? '💾 Memory') . ": {$memory}\n";
    }
    if ($bandwidth !== null) {
        $text .= ($textbotlang['keyboard']['adminVideo']['panelBandwidthLabel'] ?? '🌐 Bandwidth') . ": {$bandwidth}\n";
    }
    $text .= ($textbotlang['keyboard']['adminVideo']['panelServicesLabel'] ?? '🧾 Active Services') . ": {$services}\n";
    $text .= ($textbotlang['keyboard']['adminVideo']['panelSalesLabel'] ?? '💰 Active Sales') . ": {$sales}\n";
    if (!empty($panel['agent'])) {
        $text .= ($textbotlang['keyboard']['adminVideo']['panelAgentLabel'] ?? '🤝 Agent') . ": " . htmlspecialchars((string)$panel['agent'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "\n";
    }
    return $text;
}
...
if (in_array($text, $textadmin) || $datain == "admin") {
    ...
    return;
} elseif ($adminrulecheck['rule'] == "administrator" && ($text === ($textbotlang['keyboard']['adminVideo']['otherSettings'] ?? '') || $text === ($textbotlang['keyboard']['generalSettings'] ?? ''))) {
    // Admin menu commands must take priority over any stale state from a
    // previous multi-step form. These are reply-keyboard navigation commands,
    // so they should always open their hub and reset the admin step.
    step('home', $from_id);
    if (!isset($adminSettingsMenu) || !is_string($adminSettingsMenu) || $adminSettingsMenu === '') {
        $adminSettingsMenu = json_encode([
            'keyboard' => [
                [['text' => $textbotlang['keyboard']['shopSettings']]],
                [['text' => $textbotlang['Admin']['btnKeyboard']['managementPanel']]],
                [['text' => $textbotlang['keyboard']['paymentManagement']]],
                [['text' => $textbotlang['keyboard']['walletAdmin']['title']]],
                [['text' => $textbotlang['keyboard']['automationManagement']]],
                [['text' => $textbotlang['keyboard']['adminSecurity']]],
                [['text' => $textbotlang['keyboard']['generalSettings']]],
                [['text' => $textbotlang['Admin']['backAdminBtn']]],
            ],
            'resize_keyboard' => true,
        ], JSON_UNESCAPED_UNICODE);
    }
    sendmessage($from_id, $textbotlang['users']['selectoption'], $adminSettingsMenu, 'HTML');
    return;
} elseif ($adminrulecheck['rule'] == "administrator" && ($text === ($textbotlang['keyboard']['adminVideo']['panelManagement'] ?? '') || $text === ($textbotlang['Admin']['btnKeyboard']['managementPanel'] ?? ''))) {
    // Open the existing panel-management hub without bypassing the existing
    // panel selector/management workflow.
    step('home', $from_id);
    if (!isset($adminPanelManagementMenu) || !is_string($adminPanelManagementMenu) || $adminPanelManagementMenu === '') {
        $adminPanelManagementMenu = json_encode([
            'keyboard' => [
                [['text' => $textbotlang['keyboard']['panelListStatus']], ['text' => $textbotlang['Admin']['btnKeyboard']['addPanel']]],
                [['text' => $textbotlang['Admin']['btnKeyboard']['managementPanel']]],
    ...
    sendmessage($from_id, $textbotlang['users']['selectoption'], $adminPanelManagementMenu, 'HTML');
    return;
...
} elseif (($text == $textbotlang['Admin']['getlimitusertest']['setLimitBtn'] || $text == $textbotlang['keyboard']['setTestAccountLimitAll']) && $adminrulecheck['rule'] == "administrator") {
...
} elseif ($text == $textbotlang['keyboard']['adminVideo']['backMain'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $keyboard, 'HTML');
    step('home', $from_id);
} elseif (($text == $textbotlang['Admin']['Status']['btn'] || $datain == "stat_all_bot" || $text == $textbotlang['keyboard']['adminVideo']['quickAccount']) && $adminrulecheck['rule'] == "administrator") {
 ...
 } elseif ($text == $textbotlang['keyboard']['adminVideo']['botStatus'] && $adminrulecheck['rule'] == "administrator") {
    $botStatus = (($setting['Bot_Status'] ?? '') === 'botstatuson')
        ? ($textbotlang['Admin']['Status']['statuson'] ?? '✅ On')
        : ($textbotlang['Admin']['Status']['statusoff'] ?? '❌ Off');
    $botInfo = telegram('getMe');
    $webhookInfo = telegram('getWebhookInfo');
    $botText = $textbotlang['keyboard']['adminVideo']['botStatusHeader'] ?? '🤖 Bot Status';
    $botText .= "\n━━━━━━━━━━━━━━\n";
    $botText .= ($textbotlang['keyboard']['adminVideo']['botStatusLabel'] ?? '📊 Status') . ": {$botStatus}\n";
    $botText .= ($textbotlang['keyboard']['adminVideo']['botVersionLabel'] ?? '📦 Version') . ": <code>" . htmlspecialchars((string)$version, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "</code>\n";
    if (is_array($botInfo) && !empty($botInfo['ok']) && isset($botInfo['result'])) {
        $result = $botInfo['result'];
        $botText .= ($textbotlang['keyboard']['adminVideo']['botNameLabel'] ?? '🏷 Name') . ": " . htmlspecialchars((string)($result['first_name'] ?? '-'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "\n";
        $botText .= ($textbotlang['keyboard']['adminVideo']['botUsernameLabel'] ?? '👤 Username') . ": @" . htmlspecialchars((string)($result['username'] ?? '-'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "\n";
        $botText .= ($textbotlang['keyboard']['adminVideo']['botIdLabel'] ?? '🆔 ID') . ": <code>" . htmlspecialchars((string)($result['id'] ?? '-'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "</code>\n";
    } else {
        $botText .= ($textbotlang['keyboard']['adminVideo']['botApiUnavailable'] ?? '⚠️ Telegram bot information is unavailable.') . "\n";
    }
    if (is_array($webhookInfo) && !empty($webhookInfo['ok']) && isset($webhookInfo['result'])) {
        $webhookUrl = (string)($webhookInfo['result']['url'] ?? '');
        $botText .= ($textbotlang['keyboard']['adminVideo']['botWebhookLabel'] ?? '🔗 Webhook') . ": " . ($webhookUrl !== '' ? ($textbotlang['keyboard']['adminVideo']['botWebhookSet'] ?? '🟢 Set') : ($textbotlang['keyboard']['adminVideo']['botWebhookNotSet'] ?? '🔴 Not set')) . "\n";
    }
    sendmessage($from_id, $botText, $backadmin, 'HTML');
    step('home', $from_id);
...
elseif ($datain == "systemsms") {
    ...
    if ($typeservice == "unpinmessage") {
        ...
        if ($agent == "all") {
            $stmt = $pdo->prepare("SELECT id FROM user WHERE last_message_time < :cutoff");
            $stmt->bindValue(':cutoff', (int) $timenouser, PDO::PARAM_INT);
            $stmt->execute();
            $userslist = json_encode($stmt->fetchAll());
        } else {
            if ($typeusermessage == "all") {
    ...
    step('home', $from_id);
} elseif ($text == $textbotlang['keyboard']['supportDesk'] && $adminrulecheck['rule'] == "support") {
    $rows = [];
    try {
        $stmt = $pdo->prepare("SELECT Tracking,iduser,name_departman,text,time,status FROM support_message WHERE idsupport = :idsupport AND status IN ('Unseen','Customerresponse') ORDER BY time DESC LIMIT 20");
        $stmt->bindValue(':idsupport', $from_id, PDO::PARAM_STR);
        $stmt->execute();
        while ($ticket = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $preview = trim(preg_replace('/\\s+/', ' ', (string)($ticket['text'] ?? '')));
            if ($preview === '') $preview = $textbotlang['keyboard']['supportOpenTicket'];
            $preview = function_exists('mb_substr') ? mb_substr($preview, 0, 24) : substr($preview, 0, 24);
            $label = "🎫 #{$ticket['iduser']} — {$preview}";
            $rows[] = [['text' => $label, 'callback_data' => 'Responsesupport_' . $ticket['Tracking']]];
        }
    } catch (Throwable $e) {
        $rows = [];
    }
    if (!$rows) {
        $rows[] = [['text' => $textbotlang['keyboard']['supportNoTickets'], 'callback_data' => 'support_tickets']];
    }
    $rows[] = [['text' => $textbotlang['keyboard']['supportSearchUser'], 'callback_data' => 'support_search_user'], ['text' => $textbotlang['keyboard']['supportRefresh'], 'callback_data' => 'support_tickets']];
    $rows[] = [['text' => $textbotlang['keyboard']['supportBack'], 'callback_data' => 'support_back']];
    sendmessage($from_id, "🎧 <b>{$textbotlang['keyboard']['supportDesk']}</b>", json_encode(['inline_keyboard' => $rows], JSON_UNESCAPED_UNICODE), 'HTML');
} elseif ($datain == 'support_tickets' && $adminrulecheck['rule'] == "support") {
    $rows = [];
    try {
        $stmt = $pdo->prepare("SELECT Tracking,iduser,text FROM support_message WHERE idsupport = :idsupport AND status IN ('Unseen','Customerresponse') ORDER BY time DESC LIMIT 20");
        $stmt->bindValue(':idsupport', $from_id, PDO::PARAM_STR);
        $stmt->execute();
        while ($ticket = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $preview = trim(preg_replace('/\\s+/', ' ', (string)($ticket['text'] ?? '')));
            $preview = $preview !== '' ? (function_exists('mb_substr') ? mb_substr($preview, 0, 24) : substr($preview, 0, 24)) : $textbotlang['keyboard']['supportOpenTicket'];
            $rows[] = [['text' => "🎫 #{$ticket['iduser']} — {$preview}", 'callback_data' => 'Responsesupport_' . $ticket['Tracking']]];
        }
    } catch (Throwable $e) {}
    if (!$rows) $rows[] = [['text' => $textbotlang['keyboard']['supportNoTickets'], 'callback_data' => 'support_tickets']];
    $rows[] = [['text' => $textbotlang['keyboard']['supportSearchUser'], 'callback_data' => 'support_search_user'], ['text' => $textbotlang['keyboard']['supportRefresh'], 'callback_data' => 'support_tickets']];
    $rows[] = [['text' => $textbotlang['keyboard']['supportBack'], 'callback_data' => 'support_back']];
    Editmessagetext($from_id, $message_id, "🎧 <b>{$textbotlang['keyboard']['supportDesk']}</b>", json_encode(['inline_keyboard' => $rows], JSON_UNESCAPED_UNICODE), 'HTML');
} elseif ($datain == 'support_search_user' && $adminrulecheck['rule'] == "support") {
    sendmessage($from_id, $textbotlang['Admin']['manageUser']['getIdUserUnblock'], $backadmin, 'HTML');
    step('support_search_user', $from_id);
} elseif ($user['step'] == 'support_search_user' && $adminrulecheck['rule'] == "support") {
    $id_user = trim((string)$text);
    if (!rowExists('user', 'id', $id_user)) {
        sendmessage($from_id, $textbotlang['Admin']['notUser'], $backadmin, 'HTML');
        return;
    }
    $stmt = $pdo->prepare("SELECT username, User_Status, Balance, agent, last_message_time FROM user WHERE id = :id_user LIMIT 1");
    $stmt->bindValue(':id_user', $id_user, PDO::PARAM_STR);
    $stmt->execute();
    $u = $stmt->fetch(PDO::FETCH_ASSOC);
    $stmt = $pdo->prepare("SELECT id,name_product,Volume,Time,Status,end_of_time FROM invoice WHERE id_user = :id_user AND Status IN ('active','end_of_time','end_of_volume','sendedwarn','send_on_hold') ORDER BY id DESC LIMIT 10");
    $stmt->bindValue(':id_user', $id_user, PDO::PARAM_STR);
    $stmt->execute();
    $services = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $lines = ["👤 <b>{$u['username']}</b>", "🆔 <code>{$id_user}</code>", "📌 {$u['User_Status']}", "💰 " . number_format((float)$u['Balance'])];
    if ($services) {
        $lines[] = "\n🧩 <b>سرویس‌ها</b>";
        foreach ($services as $svc) {
            $name = htmlspecialchars((string)$svc['name_product'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
            $status = htmlspecialchars((string)$svc['Status'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
            $lines[] = "• {$name} — {$status}";
        }
    } else {
        $lines[] = "\n🧩 <b>سرویسی فعال نیست.</b>";
    }
    $supportUserKeyboard = json_encode(['inline_keyboard' => [
        [['text' => $textbotlang['keyboard']['sendMessageToUser'], 'callback_data' => 'Response_' . $id_user]],
        [['text' => $textbotlang['keyboard']['supportRefresh'], 'callback_data' => 'support_search_user_' . $id_user]],
        [['text' => $textbotlang['keyboard']['supportBack'], 'callback_data' => 'support_back']]
    ]], JSON_UNESCAPED_UNICODE);
    sendmessage($from_id, implode("\n", $lines), $supportUserKeyboard, 'HTML');
    step('home', $from_id);
} elseif (preg_match('/^support_search_user_(\\w+)$/', $datain, $supportUserMatch) && $adminrulecheck['rule'] == "support") {
    $id_user = $supportUserMatch[1];
    if (!rowExists('user', 'id', $id_user)) return;
    $stmt = $pdo->prepare("SELECT username, User_Status, Balance FROM user WHERE id = :id_user LIMIT 1");
    $stmt->bindValue(':id_user', $id_user, PDO::PARAM_STR);
    $stmt->execute();
    $u = $stmt->fetch(PDO::FETCH_ASSOC);
    $stmt = $pdo->prepare("SELECT name_product,Status FROM invoice WHERE id_user = :id_user AND Status IN ('active','end_of_time','end_of_volume','sendedwarn','send_on_hold') ORDER BY id DESC LIMIT 10");
    $stmt->bindValue(':id_user', $id_user, PDO::PARAM_STR);
    $stmt->execute();
    $services = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $lines = ["👤 <b>{$u['username']}</b>", "🆔 <code>{$id_user}</code>", "📌 {$u['User_Status']}", "💰 " . number_format((float)$u['Balance'])];
    foreach ($services as $svc) $lines[] = '• ' . htmlspecialchars((string)$svc['name_product'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . ' — ' . htmlspecialchars((string)$svc['Status'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
    Editmessagetext($from_id, $message_id, implode("\n", $lines), json_encode(['inline_keyboard' => [
        [['text' => $textbotlang['keyboard']['sendMessageToUser'], 'callback_data' => 'Response_' . $id_user]],
        [['text' => $textbotlang['keyboard']['supportRefresh'], 'callback_data' => 'support_search_user_' . $id_user], ['text' => $textbotlang['keyboard']['supportBack'], 'callback_data' => 'support_back']]
    ]], JSON_UNESCAPED_UNICODE), 'HTML');
} elseif ($datain == 'support_back' && $adminrulecheck['rule'] == "support") {
    sendmessage($from_id, $textbotlang['Admin']['backAdmin'], $keyboardadmin, 'HTML');
    ...
    step('home', $from_id);
} elseif ($text == $textbotlang['keyboard']['shopVideo']['misc'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $shopMiscKeyboard, 'HTML');
} elseif ($text == $textbotlang['keyboard']['shopVideo']['testAccount'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $Feature_status, 'HTML');
} elseif ($text == $textbotlang['keyboard']['shopVideo']['renewSettings'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $shopMiscKeyboard, 'HTML');
} elseif ($text == $textbotlang['keyboard']['shopVideo']['refundRequest'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $shopMiscKeyboard, 'HTML');
} elseif ($text == $textbotlang['keyboard']['shopVideo']['thanksMessage'] && $adminrulecheck['rule'] == "administrator") {
...
} elseif ($text == $textbotlang['keyboard']['shopVideo']['subscriptionButton'] && $adminrulecheck['rule'] == "administrator") {
    $current = select("shopSetting", "value", "Namevalue", "configshow", "select")['value'];
    $new = ($current == "onconfig") ? "offconfig" : "onconfig";
    update("shopSetting", "value", $new, "Namevalue", "configshow");
    sendmessage($from_id, $textbotlang['Admin']['cronjob']['changedData'], $shopkeyboard, 'HTML');
} elseif ($text == $textbotlang['keyboard']['shopVideo']['productTariff'] && $adminrulecheck['rule'] == "administrator") {
    $current = select("shopSetting", "value", "Namevalue", "statusshowprice", "select")['value'];
    $new = ($current == "onshowprice") ? "offshowprice" : "onshowprice";
    update("shopSetting", "value", $new, "Namevalue", "statusshowprice");
    sendmessage($from_id, $textbotlang['Admin']['cronjob']['changedData'], $shopkeyboard, 'HTML');
} elseif ($text == $textbotlang['keyboard']['shopVideo']['specialSaleDiscount'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $shopMiscKeyboard, 'HTML');
} elseif ($text == $textbotlang['keyboard']['shopVideo']['autoAccount'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $Feature_status, 'HTML');
} elseif ($text == $textbotlang['keyboard']['discountMenu'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $shopDiscountKeyboard, 'HTML');
...
} elseif ($text == $textbotlang['keyboard']['adminSecurity'] && $adminrulecheck['rule'] == "administrator") {
    // Read-only security diagnostics. No credentials or secrets are displayed.
    $adminRows = select("admin", "id_admin,rule", null, null, "fetchAll");
    $adminCount = is_array($adminRows) ? count($adminRows) : 0;
    $administratorCount = 0;
    foreach ((array) $adminRows as $adminRow) {
        if (($adminRow['rule'] ?? '') === 'administrator') {
            $administratorCount++;
        }
    }

    $pdoEmulationDisabled = false;
    try {
        $pdoEmulationDisabled = $pdo->getAttribute(PDO::ATTR_EMULATE_PREPARES) === false;
    } catch (Throwable $e) {
        $pdoEmulationDisabled = false;
    }

    $configProtected = is_file(__DIR__ . '/config.php') && !is_writable(__DIR__ . '/config.php');
    $rootHtaccess = is_file(__DIR__ . '/.htaccess');
    $apiHtaccess = is_file(__DIR__ . '/api/.htaccess');
    $apiTokenConfigured = isset($APIKEY) && is_string($APIKEY) && trim($APIKEY) !== '' && !preg_match('/^\{[^}]+\}$/', $APIKEY);
    $adminAccessOk = in_array($from_id, $admin_ids);
    $currentRule = $adminrulecheck['rule'] ?? 'unknown';

    $checks = [
        [$textbotlang['keyboard']['securityAdminAccess'], $adminAccessOk],
        [$textbotlang['keyboard']['securityAdministratorRule'], $currentRule === 'administrator'],
        [$textbotlang['keyboard']['securityPreparedStatements'], $pdoEmulationDisabled],
        [$textbotlang['keyboard']['securityConfigProtected'], $configProtected],
        [$textbotlang['keyboard']['securityRootHtaccess'], $rootHtaccess],
        [$textbotlang['keyboard']['securityApiHtaccess'], $apiHtaccess],
        [$textbotlang['keyboard']['securityApiToken'], $apiTokenConfigured],
    ];
    ...
    $summary = "🔐 <b>{$textbotlang['keyboard']['adminSecurity']}</b>\n\n" .
        "👥 {$textbotlang['keyboard']['securityAdminCount']}: <b>{$adminCount}</b>\n" .
        "🛡 {$textbotlang['keyboard']['securityAdministratorCount']}: <b>{$administratorCount}</b>\n" .
        "📊 {$textbotlang['keyboard']['securityChecks']}: <b>{$passed}/" . count($checks) . "</b>\n\n" .
        implode("\n", $lines) .
        "\n\n<i>{$textbotlang['keyboard']['securityReadOnly']}</i>";

    $securityKeyboard = json_encode([
        'inline_keyboard' => [
            [['text' => $textbotlang['keyboard']['securityRefresh'], 'callback_data' => 'admin_security_refresh']],
            [['text' => $textbotlang['keyboard']['securityBack'], 'callback_data' => 'admin_security_back']],
        ]
    ], JSON_UNESCAPED_UNICODE);

    error_log("[ADMIN_SECURITY] admin={$from_id} rule={$currentRule} checks={$passed}/" . count($checks));
    sendmessage($from_id, $summary, $securityKeyboard, 'HTML');
} elseif ($datain === 'admin_security_refresh' && $adminrulecheck['rule'] == "administrator") {
    // Refresh is intentionally read-only and recomputes every check from the live runtime.
    $adminRows = select("admin", "id_admin,rule", null, null, "fetchAll");
    $adminCount = is_array($adminRows) ? count($adminRows) : 0;
    $administratorCount = 0;
    foreach ((array) $adminRows as $adminRow) {
        if (($adminRow['rule'] ?? '') === 'administrator') { $administratorCount++; }
    }
    $pdoEmulationDisabled = false;
    try { $pdoEmulationDisabled = $pdo->getAttribute(PDO::ATTR_EMULATE_PREPARES) === false; } catch (Throwable $e) {}
    $configProtected = is_file(__DIR__ . '/config.php') && !is_writable(__DIR__ . '/config.php');
    $rootHtaccess = is_file(__DIR__ . '/.htaccess');
    $apiHtaccess = is_file(__DIR__ . '/api/.htaccess');
    $apiTokenConfigured = isset($APIKEY) && is_string($APIKEY) && trim($APIKEY) !== '' && !preg_match('/^\{[^}]+\}$/', $APIKEY);
    $checks = [
        [$textbotlang['keyboard']['securityAdminAccess'], in_array($from_id, $admin_ids)],
        [$textbotlang['keyboard']['securityAdministratorRule'], ($adminrulecheck['rule'] ?? '') === 'administrator'],
        [$textbotlang['keyboard']['securityPreparedStatements'], $pdoEmulationDisabled],
        [$textbotlang['keyboard']['securityConfigProtected'], $configProtected],
        [$textbotlang['keyboard']['securityRootHtaccess'], $rootHtaccess],
        [$textbotlang['keyboard']['securityApiHtaccess'], $apiHtaccess],
    ...
    $passed = 0; $lines = [];
    foreach ($checks as [$label, $ok]) { $passed += $ok ? 1 : 0; $lines[] = ($ok ? '🟢' : '🔴') . ' ' . $label; }
    $summary = "🔐 <b>{$textbotlang['keyboard']['adminSecurity']}</b>\n\n" .
        "👥 {$textbotlang['keyboard']['securityAdminCount']}: <b>{$adminCount}</b>\n" .
        "🛡 {$textbotlang['keyboard']['securityAdministratorCount']}: <b>{$administratorCount}</b>\n" .
        "📊 {$textbotlang['keyboard']['securityChecks']}: <b>{$passed}/" . count($checks) . "</b>\n\n" .
        implode("\n", $lines) . "\n\n<i>{$textbotlang['keyboard']['securityReadOnly']}</i>";
    $securityKeyboard = json_encode(['inline_keyboard' => [
        [['text' => $textbotlang['keyboard']['securityRefresh'], 'callback_data' => 'admin_security_refresh']],
        [['text' => $textbotlang['keyboard']['securityBack'], 'callback_data' => 'admin_security_back']],
    ]], JSON_UNESCAPED_UNICODE);
    telegram('answerCallbackQuery', ['callback_query_id' => $callback_query_id, 'text' => $textbotlang['keyboard']['securityRefresh']]);
    Editmessagetext($from_id, $message_id, $summary, $securityKeyboard, 'HTML');
} elseif ($datain === 'admin_security_back' && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['backAdmin'], $adminSettingsMenu ?? $keyboardadmin, 'HTML');
    return;
} elseif ($text == $textbotlang['keyboard']['automationManagement'] && $adminrulecheck['rule'] == "administrator") {
    $setting = select("setting", "*", null, null, "select");
    $cron = json_decode($setting['cron_status'] ?? '{}', true);
    if (!is_array($cron)) {
        $cron = [];
    }
    $cronItems = [
        'test' => $textbotlang['keyboard']['adminVideo']['automationTest'],
        'day' => $textbotlang['keyboard']['adminVideo']['automationService'],
        'volume' => $textbotlang['keyboard']['adminVideo']['automationVolume'],
        'remove' => $textbotlang['keyboard']['adminVideo']['automationRemove'],
        'remove_volume' => $textbotlang['keyboard']['adminVideo']['automationRemoveVolume'],
        'uptime_node' => $textbotlang['keyboard']['adminVideo']['automationNode'],
        'uptime_panel' => $textbotlang['keyboard']['adminVideo']['automationPanel'],
        'on_hold' => $textbotlang['keyboard']['adminVideo']['automationHold'],
    ];
    $automationKeyboard = ['keyboard' => []];
    foreach ($cronItems as $key => $label) {
        $enabled = !empty($cron[$key]);
        $statusLabel = $enabled ? $textbotlang['keyboard']['adminVideo']['automationOn'] : $textbotlang['keyboard']['adminVideo']['automationOff'];
        $automationKeyboard['keyboard'][] = [
            ['text' => $label],
            ['text' => $statusLabel, 'callback_data' => "automation_toggle-{$key}-" . ($enabled ? '1' : '0')],
        ];
    }
    $backupExists = is_file(__DIR__ . '/cronbot/backupbot.php');
    $automationKeyboard['keyboard'][] = [
        ['text' => $textbotlang['keyboard']['adminVideo']['automationBackup'] . ' ' . ($backupExists ? $textbotlang['keyboard']['adminVideo']['automationOn'] : $textbotlang['keyboard']['adminVideo']['automationMissing'])],
    ];
    $automationKeyboard['keyboard'][] = [
        ['text' => $textbotlang['keyboard']['adminVideo']['automationRefresh'], 'callback_data' => 'automation_overview'],
        ['text' => $textbotlang['keyboard']['adminVideo']['automationBack']],
    ];
    $automationKeyboard['resize_keyboard'] = true;
    $enabledCount = count(array_filter($cronItems, static function ($key) use ($cron) { return !empty($cron[$key]); }, ARRAY_FILTER_USE_KEY));
    $existingJobs = 0;
    foreach (['NoticationsService.php','payment_expire.php','expireagent.php','on_hold.php','uptime_node.php','uptime_panel.php','backupbot.php'] as $jobFile) {
        if (is_file(__DIR__ . '/cronbot/' . $jobFile)) {
            $existingJobs++;
        }
    }
    $summary = "⚙️ <b>{$textbotlang['keyboard']['adminVideo']['automationManagement']}</b>\n\n" .
        "فعال‌سازی‌های قابل‌کنترل: <b>{$enabledCount}/" . count($cronItems) . "</b>\n" .
        "کارهای خودکار موجود در پروژه: <b>{$existingJobs}</b>\n\n" .
        "ℹ️ وضعیت‌ها از تنظیمات فعلی ربات خوانده می‌شوند؛ Cron جدیدی ساخته نمی‌شود.";
    sendmessage($from_id, $summary, json_encode($automationKeyboard, JSON_UNESCAPED_UNICODE), 'HTML');
} elseif ($datain === 'automation_overview' && $adminrulecheck['rule'] == "administrator") {
    $setting = select("setting", "*", null, null, "select");
    $cron = json_decode($setting['cron_status'] ?? '{}', true);
    ...
    foreach ($items as $key => $label) {
        $enabled = !empty($cron[$key]);
        $rows[] = [['text' => $label], ['text' => $enabled ? $textbotlang['keyboard']['adminVideo']['automationOn'] : $textbotlang['keyboard']['adminVideo']['automationOff'], 'callback_data' => "automation_toggle-{$key}-" . ($enabled ? '1' : '0')]];
    }
    $rows[] = [['text' => $textbotlang['keyboard']['adminVideo']['automationBackup'] . ' ' . (is_file(__DIR__ . '/cronbot/backupbot.php') ? $textbotlang['keyboard']['adminVideo']['automationOn'] : $textbotlang['keyboard']['adminVideo']['automationMissing'])]];
    $rows[] = [['text' => $textbotlang['keyboard']['adminVideo']['automationRefresh'], 'callback_data' => 'automation_overview'], ['text' => $textbotlang['keyboard']['adminVideo']['automationBack']]];
    $settingKeyboard = ['inline_keyboard' => $rows];
    sendmessage($from_id, "⚙️ <b>{$textbotlang['keyboard']['adminVideo']['automationOverview']}</b>", json_encode($settingKeyboard, JSON_UNESCAPED_UNICODE), 'HTML');
} elseif (preg_match('/^automation_toggle-(test|day|volume|remove|remove_volume|uptime_node|uptime_panel|on_hold)-(0|1)$/', $datain, $automationMatch) && $adminrulecheck['rule'] == "administrator") {
    $setting = select("setting", "*", null, null, "select");
    $cron = json_decode($setting['cron_status'] ?? '{}', true);
    if (!is_array($cron)) { $cron = []; }
    $key = $automationMatch[1];
    $cron[$key] = $automationMatch[2] !== '1';
    update("setting", "cron_status", json_encode($cron));
    $enabledText = $cron[$key] ? $textbotlang['keyboard']['adminVideo']['automationOn'] : $textbotlang['keyboard']['adminVideo']['automationOff'];
    telegram('answerCallbackQuery', ['callback_query_id' => $callback_query_id, 'text' => $enabledText]);
    // Re-render through the same trusted setting source so the button always reflects persisted state.
    $rows = [];
    $items = [
        'test' => $textbotlang['keyboard']['adminVideo']['automationTest'],
        'day' => $textbotlang['keyboard']['adminVideo']['automationService'],
    ...
    foreach ($items as $cronKey => $label) {
        $enabled = !empty($cron[$cronKey]);
        $rows[] = [['text' => $label], ['text' => $enabled ? $textbotlang['keyboard']['adminVideo']['automationOn'] : $textbotlang['keyboard']['adminVideo']['automationOff'], 'callback_data' => "automation_toggle-{$cronKey}-" . ($enabled ? '1' : '0')]];
    }
    $rows[] = [['text' => $textbotlang['keyboard']['adminVideo']['automationBackup'] . ' ' . (is_file(__DIR__ . '/cronbot/backupbot.php') ? $textbotlang['keyboard']['adminVideo']['automationOn'] : $textbotlang['keyboard']['adminVideo']['automationMissing'])]];
    $rows[] = [['text' => $textbotlang['keyboard']['adminVideo']['automationRefresh'], 'callback_data' => 'automation_overview'], ['text' => $textbotlang['keyboard']['adminVideo']['automationBack']]];
    Editmessagetext($from_id, $message_id, "⚙️ <b>{$textbotlang['keyboard']['adminVideo']['automationOverview']}</b>\n\nوضعیت ذخیره شد.", json_encode(['inline_keyboard' => $rows], JSON_UNESCAPED_UNICODE), 'HTML');
} elseif ($text == $textbotlang['keyboard']['automationBack'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $adminSettingsMenu, 'HTML');
} elseif ($text == $textbotlang['keyboard']['paymentManagement'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $adminPaymentManagementMenu, 'HTML');
} elseif ($text == $textbotlang['keyboard']['walletAdmin']['title'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $adminWalletManagementMenu, 'HTML');
} elseif ($text == $textbotlang['keyboard']['walletAdmin']['chargeUser'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['manageUser']['getIdUserUnblock'], $backadmin, 'HTML');
    step('show_info', $from_id);
} elseif ($text == $textbotlang['keyboard']['walletAdmin']['decreaseUser'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['negativeBalance'], $backadmin, 'HTML');
    step('Negative_Balance', $from_id);
} elseif ($text == $textbotlang['keyboard']['walletAdmin']['groupCharge'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['Balance']['addAllBalance'], $backadmin, 'HTML');
    step('add_Balance_all', $from_id);
} elseif ($text == $textbotlang['keyboard']['walletAdmin']['usersWithBalance'] && $adminrulecheck['rule'] == "administrator") {
    $walletInline = json_encode(['inline_keyboard' => [
    ...
    sendmessage($from_id, $textbotlang['Admin']['selectOption3'], $walletInline, 'HTML');
} elseif ($text == $textbotlang['keyboard']['walletAdmin']['usersWithNegativeBalance'] && $adminrulecheck['rule'] == "administrator") {
    $walletInline = json_encode(['inline_keyboard' => [
        [['text' => $textbotlang['keyboard']['usersWithNegativeBalance'], 'callback_data' => 'zerobalance']],
        [['text' => $textbotlang['keyboard']['backToMain'], 'callback_data' => 'backuser']],
    ]]);
    sendmessage($from_id, $textbotlang['Admin']['selectOption3'], $walletInline, 'HTML');
} elseif ($text == $textbotlang['keyboard']['walletAdmin']['resetUser'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['manageUser']['getIdUserUnblock'], $backadmin, 'HTML');
    step('show_info', $from_id);
} elseif ($text == $textbotlang['keyboard']['walletAdmin']['backSettings'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $adminSettingsMenu, 'HTML');
} elseif (($text == $textbotlang['keyboard']['generalSettings'] || $text == $textbotlang['keyboard']['adminVideo']['otherSettings']) && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $adminSettingsMenu, 'HTML');
    ...
    step('home', $from_id);
} elseif ($text == $textbotlang['keyboard']['adminVideo']['panelManagement'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $adminPanelManagementMenu, 'HTML');
 } elseif (($text == $textbotlang['keyboard']['adminVideo']['panelStatus'] || $text == $textbotlang['keyboard']['panelListStatus']) && $adminrulecheck['rule'] == "administrator") {
    $panels = $pdo->query("SELECT * FROM marzban_panel ORDER BY name_panel ASC")->fetchAll(PDO::FETCH_ASSOC);
    if (!$panels) {
        sendmessage($from_id, $textbotlang['keyboard']['adminVideo']['panelStatusEmpty'] ?? '⚠️ No panels are configured.', $backadmin, 'HTML');
        step('home', $from_id);
        return;
    }
    // Deliberately send one message per panel, matching the reference behavior
    // when two or more panels are configured.
    foreach ($panels as $panel) {
        sendmessage($from_id, admirall_admin_panel_status_report($panel, $textbotlang, $pdo), $backadmin, 'HTML');
    }
    step('home', $from_id);
    ...
    step('home', $from_id);
} elseif ($text == $textbotlang['keyboard']['adminVideo']['viewUserInfo'] && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['manageUser']['getIdUserUnblock'], $backadmin, 'HTML');
    step('show_info', $from_id);
} elseif ($text == $textbotlang['Admin']['btnKeyboard']['manageUser'] || $datain == "backlistuser" || ($text == $textbotlang['keyboard']['adminVideo']['userServices'] && $adminrulecheck['rule'] == "administrator")) {
    $keyboardtypelistuser = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $textbotlang['keyboard']['adminVideo']['serviceManagement'], 'callback_data' => 'admin-service-menu'],
            ],
            [
                ['text' => $textbotlang['keyboard']['notificationCenter'], 'callback_data' => 'admin-notification-hub'],
            ],
            [
                ['text' => $textbotlang['keyboard']['usersWithBalance'], 'callback_data' => "balanceuserlist"],
            ],
 ...
 } elseif ($datain == "admin-notification-hub" && $adminrulecheck['rule'] == "administrator") {
    // M20: Notification Center. Reuses the existing bulk-message state machine;
    // this hub only reorganizes the existing actions and adds no new delivery backend.
    $notificationMenu = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $textbotlang['keyboard']['broadcastSend'], 'callback_data' => 'typeservice-sendmessage'],
                ['text' => $textbotlang['keyboard']['broadcastForward'], 'callback_data' => 'typeservice-forwardmessage'],
            ...
            [
                ['text' => $textbotlang['keyboard']['notificationUsers'], 'callback_data' => 'systemsms'],
            ],
            [
                ['text' => $textbotlang['keyboard']['backToUserServices'], 'callback_data' => 'backlistuser'],
            ],
        ]
    ], JSON_UNESCAPED_UNICODE);
    Editmessagetext($from_id, $message_id, $textbotlang['Admin']['notificationCenter']['title'], $notificationMenu);
} elseif ($datain == "admin-service-menu" && $adminrulecheck['rule'] == "administrator") {
    $adminServiceMenu = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $textbotlang['keyboard']['adminVideo']['serviceSearch'], 'callback_data' => 'searchorder'],
                ['text' => $textbotlang['keyboard']['adminVideo']['serviceActive'], 'callback_data' => 'admin-service-list-active'],
            ],
            [
                ['text' => $textbotlang['keyboard']['adminVideo']['serviceFinished'], 'callback_data' => 'admin-service-list-finished'],
            ],
            [
                ['text' => $textbotlang['keyboard']['adminVideo']['serviceBack'], 'callback_data' => 'backlistuser'],
            ],
        ]
    ], JSON_UNESCAPED_UNICODE);
    Editmessagetext($from_id, $message_id, $textbotlang['Admin']['selectOption3'], $adminServiceMenu);
} elseif (preg_match('/^admin-service-list-(active|finished)$/', $datain, $serviceListMatch) && $adminrulecheck['rule'] == "administrator") {
    $statusLabel = $serviceListMatch[1] === 'active'
        ? $textbotlang['keyboard']['adminVideo']['serviceActive']
        : $textbotlang['keyboard']['adminVideo']['serviceFinished'];
    if ($serviceListMatch[1] === 'active') {
        $stmt = $pdo->query("SELECT id_invoice, username, name_product, Status, Service_time, Volume FROM invoice WHERE Status = 'active' ORDER BY id_invoice DESC LIMIT 20");
    } else {
        $stmt = $pdo->query("SELECT id_invoice, username, name_product, Status, Service_time, Volume FROM invoice WHERE Status IN ('end_of_time','end_of_volume','expired','disablebyadmin','removebyadmin') ORDER BY id_invoice DESC LIMIT 20");
    }
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $serviceKeyboard = ['inline_keyboard' => []];
    foreach ($rows as $row) {
        $label = trim($row['username'] . ' — ' . $row['name_product']);
        if ($label === '—') {
            $label = (string)$row['id_invoice'];
        }
        $serviceKeyboard['inline_keyboard'][] = [[
            'text' => mb_substr($label, 0, 55),
            'callback_data' => 'manageinvoice_' . $row['id_invoice']
        ]];
    }
    $serviceKeyboard['inline_keyboard'][] = [[
        'text' => $textbotlang['keyboard']['adminVideo']['serviceBack'],
        'callback_data' => 'admin-service-menu'
    ]];
    $serviceText = $statusLabel . "\n\n" . ($rows ? $textbotlang['keyboard']['adminVideo']['serviceSelectPrompt'] : $textbotlang['keyboard']['adminVideo']['serviceNone']);
    Editmessagetext($from_id, $message_id, $serviceText, json_encode($serviceKeyboard, JSON_UNESCAPED_UNICODE));
...
if ($datain == "settimecornremove" && $adminrulecheck['rule'] == "administrator") {
    ...
    step("home", $from_id);
} elseif (($text == $textbotlang['keyboard']['botReport'] || $text == $textbotlang['keyboard']['adminVideo']['reports']) && $adminrulecheck['rule'] == "administrator") {
    // M21: Reports Hub - keeps the existing statistics engine, but exposes it as a focused admin workspace.
    $reportKeyboard = json_encode(['inline_keyboard' => [
        [
            ['text' => '📊 نمای کلی', 'callback_data' => 'report_overview'],
            ['text' => '💰 فروش و درآمد', 'callback_data' => 'report_sales']
        ],
        [
            ['text' => '👥 کاربران', 'callback_data' => 'report_users'],
            ['text' => '🧩 سرویس‌ها', 'callback_data' => 'report_services']
        ],
        [
            ['text' => '🏆 محصولات پرفروش', 'callback_data' => 'report_top_products'],
            ['text' => '⭐ کاربران برتر', 'callback_data' => 'report_top_users']
        ],
        [
            ['text' => '⏱ گزارش‌های زمانی', 'callback_data' => 'report_time'],
            ['text' => '🧾 تراکنش‌ها', 'callback_data' => 'report_payments']
        ],
        [['text' => '🔙 بازگشت', 'callback_data' => 'report_back']]
    ]], JSON_UNESCAPED_UNICODE);
    $reportText = "📈 <b>مرکز گزارشات</b>\n\nاز اینجا می‌توانی وضعیت فروش، کاربران، سرویس‌ها و عملکرد محصولات را بدون ورود به چند منوی جداگانه بررسی کنی.";
    sendmessage($from_id, $reportText, $reportKeyboard, 'HTML');
    step('home', $from_id);
} elseif ($datain == 'report_overview' && $adminrulecheck['rule'] == 'administrator') {
    $users = (int)$pdo->query("SELECT COUNT(*) FROM user")->fetchColumn();
    $active = (int)$pdo->query("SELECT COUNT(*) FROM invoice WHERE Status IN ('active','end_of_time','end_of_volume','sendedwarn','send_on_hold') AND name_product != '" . $textbotlang['common']['labels']['testServiceName'] . "'")->fetchColumn();
    $expired = (int)$pdo->query("SELECT COUNT(*) FROM invoice WHERE Status IN ('expired','disablebyadmin','removebyadmin') AND name_product != '" . $textbotlang['common']['labels']['testServiceName'] . "'")->fetchColumn();
    $orders = (int)$pdo->query("SELECT COUNT(*) FROM invoice WHERE Status != 'Unpaid' AND name_product != '" . $textbotlang['common']['labels']['testServiceName'] . "'")->fetchColumn();
    $revenue = (float)$pdo->query("SELECT COALESCE(SUM(price_product),0) FROM invoice WHERE Status != 'Unpaid' AND name_product != '" . $textbotlang['common']['labels']['testServiceName'] . "'")->fetchColumn();
    $balance = (float)$pdo->query("SELECT COALESCE(SUM(Balance),0) FROM user")->fetchColumn();
    $txt = "📊 <b>نمای کلی سیستم</b>\n\n👥 کاربران: <b>" . number_format($users) . "</b>\n🟢 سرویس‌های فعال: <b>" . number_format($active) . "</b>\n⚫ سرویس‌های پایان‌یافته: <b>" . number_format($expired) . "</b>\n🧾 سفارش‌های ثبت‌شده: <b>" . number_format($orders) . "</b>\n💰 فروش ثبت‌شده: <b>" . number_format($revenue) . "</b>\n💳 مجموع موجودی کاربران: <b>" . number_format($balance) . "</b>";
    Editmessagetext($from_id, $message_id, $txt, $keyboard_stat, 'HTML');
} elseif ($datain == 'report_sales' && $adminrulecheck['rule'] == 'administrator') {
    $today = $pdo->query("SELECT COUNT(*) c, COALESCE(SUM(price_product),0) s FROM invoice WHERE time_sell >= UNIX_TIMESTAMP(CURDATE()) AND Status != 'Unpaid' AND name_product != '" . $textbotlang['common']['labels']['testServiceName'] . "'")->fetch(PDO::FETCH_ASSOC);
    $month = $pdo->query("SELECT COUNT(*) c, COALESCE(SUM(price_product),0) s FROM invoice WHERE time_sell >= UNIX_TIMESTAMP(DATE_FORMAT(CURDATE(),'%Y-%m-01')) AND Status != 'Unpaid' AND name_product != '" . $textbotlang['common']['labels']['testServiceName'] . "'")->fetch(PDO::FETCH_ASSOC);
    $txt = "💰 <b>فروش و درآمد</b>\n\n📅 امروز: <b>" . number_format((int)$today['c']) . "</b> سفارش — <b>" . number_format((float)$today['s']) . "</b>\n🗓 این ماه: <b>" . number_format((int)$month['c']) . "</b> سفارش — <b>" . number_format((float)$month['s']) . "</b>\n\nبرای تحلیل دقیق‌تر از گزارش‌های زمانی استفاده کن.";
    Editmessagetext($from_id, $message_id, $txt, $keyboard_stat, 'HTML');
} elseif ($datain == 'report_users' && $adminrulecheck['rule'] == 'administrator') {
    $newToday = (int)$pdo->query("SELECT COUNT(*) FROM user WHERE register >= UNIX_TIMESTAMP(CURDATE()) AND register != 'none'")->fetchColumn();
    $withBalance = (int)$pdo->query("SELECT COUNT(*) FROM user WHERE Balance > 0")->fetchColumn();
    $negative = (int)$pdo->query("SELECT COUNT(*) FROM user WHERE Balance < 0")->fetchColumn();
    $txt = "👥 <b>گزارش کاربران</b>\n\n🆕 ثبت‌نام امروز: <b>" . number_format($newToday) . "</b>\n💳 دارای موجودی: <b>" . number_format($withBalance) . "</b>\n⚠️ موجودی منفی: <b>" . number_format($negative) . "</b>";
    Editmessagetext($from_id, $message_id, $txt, $keyboard_stat, 'HTML');
} elseif ($datain == 'report_services' && $adminrulecheck['rule'] == 'administrator') {
    $rows = $pdo->query("SELECT Status, COUNT(*) c FROM invoice GROUP BY Status ORDER BY c DESC")->fetchAll(PDO::FETCH_ASSOC);
    $txt = "🧩 <b>وضعیت سرویس‌ها</b>\n\n";
    foreach ($rows as $r) { $txt .= '• ' . htmlspecialchars($r['Status']) . ': <b>' . number_format((int)$r['c']) . "</b>\n"; }
    Editmessagetext($from_id, $message_id, $txt, $keyboard_stat, 'HTML');
} elseif ($datain == 'report_top_products' && $adminrulecheck['rule'] == 'administrator') {
    $testName = $textbotlang['common']['labels']['testServiceName'];
    $stmtTop = $pdo->prepare("SELECT name_product, COUNT(*) c, COALESCE(SUM(price_product),0) revenue FROM invoice WHERE Status != 'Unpaid' AND name_product != :test GROUP BY name_product ORDER BY c DESC LIMIT 10");
    $stmtTop->execute([':test'=>$testName]);
    $rows = $stmtTop->fetchAll(PDO::FETCH_ASSOC);
    $txt = "🏆 <b>محصولات پرفروش</b>\n\n";
    $i=1; foreach ($rows as $r) { $txt .= $i++ . '. ' . htmlspecialchars($r['name_product']) . ' — <b>' . number_format((int)$r['c']) . '</b> فروش / ' . number_format((float)$r['revenue']) . "\n"; }
    if (!$rows) $txt .= 'داده‌ای برای نمایش وجود ندارد.';
    Editmessagetext($from_id, $message_id, $txt, $keyboard_stat, 'HTML');
} elseif ($datain == 'report_top_users' && $adminrulecheck['rule'] == 'administrator') {
    $testName = $textbotlang['common']['labels']['testServiceName'];
    $stmtTop = $pdo->prepare("SELECT id_user, MAX(username) username, COUNT(*) c, COALESCE(SUM(price_product),0) revenue FROM invoice WHERE Status != 'Unpaid' AND name_product != :test GROUP BY id_user ORDER BY revenue DESC LIMIT 10");
    $stmtTop->execute([':test'=>$testName]);
    $rows = $stmtTop->fetchAll(PDO::FETCH_ASSOC);
    $txt = "⭐ <b>کاربران برتر خرید</b>\n\n";
    $i=1; foreach ($rows as $r) { $name = $r['username'] ?: ('ID ' . $r['id_user']); $txt .= $i++ . '. ' . htmlspecialchars($name) . ' — <b>' . number_format((float)$r['revenue']) . '</b> / ' . number_format((int)$r['c']) . " خرید\n"; }
    if (!$rows) $txt .= 'داده‌ای برای نمایش وجود ندارد.';
    Editmessagetext($from_id, $message_id, $txt, $keyboard_stat, 'HTML');
} elseif ($datain == 'report_time' && $adminrulecheck['rule'] == 'administrator') {
    Editmessagetext($from_id, $message_id, '⏱ <b>گزارش‌های زمانی</b>\n\nبازه موردنظر را انتخاب کن:', $keyboard_stat, 'HTML');
} elseif ($datain == 'report_payments' && $adminrulecheck['rule'] == 'administrator') {
    $rows = $pdo->query("SELECT Payment_Method, COUNT(*) c, COALESCE(SUM(price),0) total FROM Payment_report WHERE payment_Status='paid' GROUP BY Payment_Method ORDER BY total DESC")->fetchAll(PDO::FETCH_ASSOC);
    $txt = "🧾 <b>تراکنش‌های پرداخت</b>\n\n";
    foreach ($rows as $r) { $txt .= '• ' . htmlspecialchars($r['Payment_Method']) . ': ' . number_format((int)$r['c']) . ' تراکنش — <b>' . number_format((float)$r['total']) . "</b>\n"; }
    if (!$rows) $txt .= 'تراکنش پرداخت‌شده‌ای پیدا نشد.';
    Editmessagetext($from_id, $message_id, $txt, $keyboard_stat, 'HTML');
} elseif ($datain == 'report_back' && $adminrulecheck['rule'] == 'administrator') {
    sendmessage($from_id, $textbotlang['Admin']['report']['botReportIntro'], $backadmin, 'HTML');
    step('home', $from_id);
    ...
    step('home', $from_id);
    sendmessage($from_id, $textbotlang['Admin']['Help']['tutorialSaved'], $CartManage, 'HTML');
 } elseif ($datain == "admin-other-settings" && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['users']['selectoption'], $adminSettingsMenu, 'HTML');
 } elseif ($text == $textbotlang['keyboard']['resellerManagement'] && $adminrulecheck['rule'] == "administrator") {
    // M19: reseller management hub. It deliberately reuses the existing agent/botsaz
    // data model and exposes a useful read-only dashboard before destructive actions.
    $agentCounts = [
        'all' => 0,
        'normal' => 0,
        'advanced' => 0,
    ];
    $stmtAgent = $pdo->query("SELECT agent, COUNT(*) AS c FROM user WHERE agent IN ('n','n2') GROUP BY agent");
    foreach ($stmtAgent->fetchAll(PDO::FETCH_ASSOC) as $row) {
        if ($row['agent'] === 'n') $agentCounts['normal'] = (int)$row['c'];
        if ($row['agent'] === 'n2') $agentCounts['advanced'] = (int)$row['c'];
    }
    $agentCounts['all'] = $agentCounts['normal'] + $agentCounts['advanced'];
    $botCount = (int)$pdo->query("SELECT COUNT(*) FROM botsaz")->fetchColumn();
    $pendingAgentRequests = 0;
    try {
        $stmtReq = $pdo->query("SELECT COUNT(*) FROM user WHERE agent = 'f' AND request_agent IS NOT NULL AND request_agent <> ''");
        $pendingAgentRequests = (int)$stmtReq->fetchColumn();
    } catch (Throwable $e) {
        // Older installations may not have request_agent; do not break the admin menu.
        $pendingAgentRequests = 0;
    }
    $resellerText = "<b>🤝 مدیریت نمایندگان</b>\n\n" .
        "👥 نمایندگان عادی: <b>{$agentCounts['normal']}</b>\n" .
        "⭐ نمایندگان پیشرفته: <b>{$agentCounts['advanced']}</b>\n" .
        "🤝 مجموع نمایندگان: <b>{$agentCounts['all']}</b>\n" .
        "🤖 ربات‌های فروش نمایندگان: <b>{$botCount}</b>\n" .
        "📨 درخواست‌های در انتظار: <b>{$pendingAgentRequests}</b>\n\n" .
        "از این بخش مدیریت نمایندگی را انجام دهید. قابلیت‌های موجود پروژه مستقیماً استفاده می‌شوند.";
    $resellerMenu = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $textbotlang['keyboard']['agentList'], 'callback_data' => 'agentlistusers'],
                ['text' => '📊 آمار نمایندگان', 'callback_data' => 'admin-reseller-dashboard'],
            ],
            [
                ['text' => $textbotlang['keyboard']['agentMembershipFee'], 'callback_data' => 'admin-reseller-membership-fee'],
                ['text' => $textbotlang['keyboard']['reWebhookAgentBots'], 'callback_data' => 'admin-reseller-webhooks'],
            ],
            [
                ['text' => '👤 مدیریت نماینده از کاربران', 'callback_data' => 'admin-reseller-users'],
            ],
            [
                ['text' => $textbotlang['users']['status']['backinfo'], 'callback_data' => 'admin-other-settings'],
            ]
        ]
    ]);
    sendmessage($from_id, $resellerText, $resellerMenu, 'HTML');
 } elseif ($datain == "admin-reseller-dashboard" && $adminrulecheck['rule'] == "administrator") {
    // Re-open the same dashboard without changing any data.
    $normalAgents = (int)$pdo->query("SELECT COUNT(*) FROM user WHERE agent = 'n'")->fetchColumn();
    $advancedAgents = (int)$pdo->query("SELECT COUNT(*) FROM user WHERE agent = 'n2'")->fetchColumn();
    $botCount = (int)$pdo->query("SELECT COUNT(*) FROM botsaz")->fetchColumn();
    $agentUsers = (int)$pdo->query("SELECT COUNT(*) FROM user WHERE agent IN ('n','n2')")->fetchColumn();
    $textDashboard = "<b>📊 داشبورد نمایندگان</b>\n\n" .
        "👥 نماینده عادی: <b>{$normalAgents}</b>\n" .
        "⭐ نماینده پیشرفته: <b>{$advancedAgents}</b>\n" .
        "👤 کاربران وابسته به نمایندگان: <b>{$agentUsers}</b>\n" .
        "🤖 ربات فروش ساخته‌شده: <b>{$botCount}</b>";
    $kb = json_encode(['inline_keyboard' => [
        [['text' => $textbotlang['keyboard']['agentList'], 'callback_data' => 'agentlistusers']],
        [['text' => '🔙 بازگشت به مدیریت نمایندگان', 'callback_data' => 'admin-reseller-menu']],
    ]]);
    Editmessagetext($from_id, $message_id, $textDashboard, $kb);
 } elseif ($datain == "admin-reseller-menu" && $adminrulecheck['rule'] == "administrator") {
    $adminResellerText = "<b>🤝 مدیریت نمایندگان</b>\n\nیک بخش مدیریتی متمرکز برای نمایندگان، آمار، ربات‌های فروش و تنظیمات عضویت.";
    $kb = json_encode(['inline_keyboard' => [
        [['text' => $textbotlang['keyboard']['agentList'], 'callback_data' => 'agentlistusers'], ['text' => '📊 آمار نمایندگان', 'callback_data' => 'admin-reseller-dashboard']],
        [['text' => $textbotlang['keyboard']['agentMembershipFee'], 'callback_data' => 'admin-reseller-membership-fee'], ['text' => $textbotlang['keyboard']['reWebhookAgentBots'], 'callback_data' => 'admin-reseller-webhooks']],
        [['text' => '👤 مدیریت نماینده از کاربران', 'callback_data' => 'admin-reseller-users']],
    ]]);
    Editmessagetext($from_id, $message_id, $adminResellerText, $kb);
 } elseif ($datain == "admin-reseller-membership-fee" && $adminrulecheck['rule'] == "administrator") {
    sendmessage($from_id, $textbotlang['Admin']['agent']['askMembershipFee'], $backadmin, 'HTML');
    step("getpricereqagent", $from_id);
 } elseif ($datain == "admin-reseller-webhooks" && $adminrulecheck['rule'] == "administrator") {
    $bots_agent = select("botsaz", "*", null, null, "fetchAll");
    if (count($bots_agent) == 0) {
        sendmessage($from_id, $textbotlang['Admin']['agentbot']['noneFound'], $backadmin, 'HTML');
        return;
    }
    ...
    sendmessage($from_id, $textbotlang['Admin']['agentbot']['webhookDone'], $backadmin, 'HTML');
 } elseif ($datain == "admin-reseller-users" && $adminrulecheck['rule'] == "administrator") {
    // Reuse the existing, tested user-management surface instead of creating a second reseller user engine.
    $keyboardResellerUsers = json_encode(['inline_keyboard' => [
        [['text' => $textbotlang['keyboard']['agentList'], 'callback_data' => 'agentlistusers']],
        [['text' => $textbotlang['keyboard']['searchUserBtn'], 'callback_data' => 'searchuser']],
        [['text' => $textbotlang['users']['status']['backinfo'], 'callback_data' => 'admin-reseller-menu']],
    ]]);
    Editmessagetext($from_id, $message_id, "<b>👤 کاربران و نمایندگان</b>\n\nنماینده را از لیست انتخاب کنید تا همان ابزار مدیریت کاربر فعلی باز شود.", $keyboardResellerUsers);
...
} elseif (($text == $textbotlang['keyboard']['autoConfirmNoCheck'] || $text == $textbotlang['keyboard']['adminVideo']['autoConfirm']) && $adminrulecheck['rule'] == "administrator") {
...
} elseif ($text == $textbotlang['keyboard']['pendingReceipts'] || $text == $textbotlang['keyboard']['adminVideo']['pendingReceipts']) {
