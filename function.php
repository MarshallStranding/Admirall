...
function DirectPayment($order_id, $image = 'images.jpg')
{
    ...
    if ($steppay[0] == "getconfigafterpay") {
        ...
        // Clear placeholders – configs will be sent one-by-one by sendMessageService
        $textcreatuser = str_replace(['{config}', '{links}', '{links2}'], '', $textcreatuser);
...
/**
 * Send a purchased/service delivery message and then deliver every config
 * as an individual Telegram message. No config keyboard is ever created.
 */
function sendMessageService($panel_info, $config, $sub_link, $username_service, $reply_markup, $caption, $invoice_id, $user_id = null, $image = 'images.jpg')
{
    ...
    if (function_exists('isTelegramChatIdEmpty') && isTelegramChatIdEmpty($user_id)) {
        return;
    }

    // 1) Service information caption (tutorial keyboard allowed only here)
    sendmessage($user_id, $caption, $reply_markup, 'HTML');

    // 2) Subscription URL + QR when panel provides one
    if (!empty($sub_link) && isset($panel_info['sublink']) && $panel_info['sublink'] === 'onsublink') {
        // Special handling for WGDashboard (send as .conf document)
        if (isset($panel_info['type']) && $panel_info['type'] === 'WGDashboard') {
            ...
            if (@file_put_contents($urlimage, $sub_link) !== false) {
                telegram('senddocument', [
                    ...
                    'caption' => "🔗 <b>فایل کانفیگ WireGuard</b>",
                    'parse_mode' => 'HTML',
                ]);
                @unlink($urlimage);
            } else {
                sendSubscriptionDelivery($user_id, $sub_link, $invoice_id, $image);
            }
        } else {
            sendSubscriptionDelivery($user_id, $sub_link, $invoice_id, $image);
        }
    ...
    // 3) Every config delivered separately with its real name
    if (isset($panel_info['config']) && $panel_info['config'] === 'onconfig' && !empty($config)) {
        sendConfigsDirectly($user_id, $config, $invoice_id, $username_service, true);
    }
...
/**
 * Send subscription link and its QR as a standalone delivery message.
 */
function sendSubscriptionDelivery($user_id, $subscription_url, $invoice_id = '', $image = 'images.jpg')
{
    $subscription_url = trim((string)$subscription_url);
    if ($subscription_url === '') {
        return false;
    }

    $safeUrl = htmlspecialchars($subscription_url, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
    $caption = "🔗 <b>لینک اشتراک سرویس</b>\n\n<code>{$safeUrl}</code>";

    $random = bin2hex(random_bytes(4));
    $urlimage = qrTempPath("{$user_id}_sub_{$invoice_id}_{$random}.png");
    $qrCode = createqrcode($subscription_url);
    ...
    if ($qrCode !== null && @file_put_contents($urlimage, $qrCode->getString()) !== false) {
        ...
        sendmessage($user_id, $caption, null, 'HTML');
    }
...
/**
 * Deliver all configs one-by-one. There is intentionally NO config keyboard.
 * A single back button is sent only after all configs have been delivered.
 */
function sendConfigsDirectly($user_id, $configs, $invoice_id = '', $username_service = '', $send_back_button = true)
{
    global $textbotlang;
    ...
    foreach ($configs as $config) {
        ...
        // Split multi-link blocks if present
        if (preg_match_all('#(?:vless|vmess|trojan|ss|ssr|hysteria2?|tuic|wireguard)://[^\s]+#i', $config, $mm)) {
            foreach ($mm[0] as $one) {
                $cleanConfigs[] = $one;
            }
        } elseif (preg_match('#^[a-z0-9]+://#i', $config)) {
            $cleanConfigs[] = $config;
        } else {
            $cleanConfigs[] = $config;
        }
    ...
    $cleanConfigs = array_values(array_unique($cleanConfigs));
    $total = count($cleanConfigs);

    if ($total === 0) {
        sendmessage($user_id, '❌ برای این سرویس کانفیگی برای ارسال پیدا نشد.', null, 'HTML');
        return false;
    }

    foreach ($cleanConfigs as $index => $config) {
        $number = $index + 1;
        $name = extract_config_name($config, $number);
        $safeName = htmlspecialchars($name, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
        $safeConfig = htmlspecialchars($config, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');

        $message = "<b>{$safeName}</b>\n\n<code>{$safeConfig}</code>";

        // ارسال با اسم کانفیگ بدون دکمه (دکمه بازگشت فقط در آخر)
        sendmessage($user_id, $message, null, 'HTML');

        if ($number < $total) {
            usleep(180000); // anti-flood
        }
    }

    if ($send_back_button && !empty($invoice_id)) {
        $backText = $textbotlang['users']['status']['backinfo'] ?? '↩️ بازگشت به سرویس';
        $backMarkup = json_encode([
            'inline_keyboard' => [[
                ['text' => $backText, 'callback_data' => 'product_' . $invoice_id]
            ]]
        ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

        sendmessage(
            $user_id,
            "✅ <b>همه کانفیگ‌ها ارسال شدند.</b>\n\nبرای استفاده، کانفیگ موردنظر را کپی کنید.",
            $backMarkup,
            'HTML'
        );
    } elseif ($send_back_button) {
        sendmessage(
            $user_id,
            "✅ <b>همه کانفیگ‌ها ارسال شدند.</b>\n\nبرای استفاده، کانفیگ موردنظر را کپی کنید.",
            null,
...
/**
 * Extract a human-readable name from VLESS/VMess/Trojan/SS links.
 * Falls back safely to protocol + sequence number.
 */
function extract_config_name($config, $index = 1)
{
    $fallback = 'کانفیگ ' . (int)$index;
    ...
    try {
        ...
        if (is_array($parts) && !empty($parts['fragment'])) {
            $fragment = urldecode($parts['fragment']);
            if ($fragment !== '') {
                return $fragment;
            }
        }

        $protocol = strtolower((string)strtok($config, ':'));

        // VMess stores the display name in the JSON field "ps"
        if ($protocol === 'vmess' && strpos($config, '://') !== false) {
            $encoded = substr($config, strpos($config, '://') + 3);
        ...
        // Some providers put the name after '#'
        $hashPos = strrpos($config, '#');
        if ($hashPos !== false && $hashPos < strlen($config) - 1) {
            $fragment = urldecode(substr($config, $hashPos + 1));
            if ($fragment !== '') {
                return $fragment;
            }
        }
        ...
        if ($protocol !== '') {
            return strtoupper($protocol) . ' ' . (int)$index;
        }
    } catch (Throwable $e) {
