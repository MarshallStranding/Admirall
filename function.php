...
function sendMessageService($panel_info, $config, $sub_link, $username_service, $reply_markup, $caption, $invoice_id, $user_id = null, $image = 'images.jpg')
{
    ...
    // 1) Service information. The supplied keyboard is allowed here
    // (e.g. tutorial button), but config messages themselves have no keyboard.
    sendmessage($user_id, $caption, $reply_markup, 'HTML');

    // 2) Subscription URL + QR, when the panel provides one.
    ...
    // 3) Every config is delivered separately with its real name.
    if (isset($panel_info['config']) && $panel_info['config'] === 'onconfig' && is_array($config)) {
...
function sendConfigsDirectly($user_id, $configs, $invoice_id = '', $username_service = '', $send_back_button = true)
{
    ...
    foreach ($cleanConfigs as $index => $config) {
        ...
        $safeUsername = htmlspecialchars((string)$username_service, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');

        $message = "🔐 <b>کانفیگ {$number} از {$total}</b>\n";
        $message .= "📌 <b>نام:</b> {$safeName}\n";
        if ($safeUsername !== '') {
            $message .= "👤 <b>سرویس:</b> <code>{$safeUsername}</code>\n";
        }
        $message .= "\n<code>{$safeConfig}</code>";

        // No reply_markup here. This is deliberate.
        sendmessage($user_id, $message, null, 'HTML');

        // Small delay prevents Telegram rate-limit issues when a service has many configs.
        if ($number < $total) {
            usleep(180000);
        }
    }
...
function extract_config_name($config, $index = 1)
{
    ...
    try {
        ...
        // Some providers put the name after '#', even when parse_url cannot parse it.
